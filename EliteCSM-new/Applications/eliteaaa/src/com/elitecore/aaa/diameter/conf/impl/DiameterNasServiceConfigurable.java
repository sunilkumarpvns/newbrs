...
@ConfigurationProperties(moduleName ="NAS_APPLICATION_CONFIGURABLE",readWith = XMLReader.class, reloadWith = XMLReader.class, synchronizeKey =AAAServerConstants.DIA_NAS_SERVICE_ID)
@XMLProperties(name = "nas-application", schemaDirectories = {"system","schema"}, configDirectories = {"conf","diameter"})
public class DiameterNasServiceConfigurable extends DiameterServiceConfigurable{
	
	private ApplicationEnum supportedApplication;

	public DiameterNasServiceConfigurable() {
	}
	...
	public String toString () {
		...
		out.println();
		out.println();
		out.println(" -- Diameter NAS Service Configuration -- ");
		out.println();
		out.println("    Application Id = " + getSupportedApplication());
	...
	private void postReadApplicationEnums() {
		...
		if (Strings.isNullOrBlank(getStrApplicationId())) {
			supportedApplication = ApplicationIdentifier.NASREQ;  
		} else {
			...
			try {
				...
				supportedApplication = DiameterUtility.createApplicationEnumLeniently(applicationId, vendorId, ServiceTypes.BOTH, Application.NASREQ);
	...
	public ServiceTypeConstants getServiceType() {
		return ServiceTypeConstants.DIA_NAS;
	}
}
