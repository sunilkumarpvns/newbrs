...
import com.elitecore.aaa.core.config.RRDDetail;
...
@ConfigurationProperties(moduleName ="RAD_AUTH_CONFIGURABLE",readWith = XMLReader.class, reloadWith = XMLReader.class, synchronizeKey ="RAD_AUTH")
@XMLProperties(name = "rad-auth", schemaDirectories = {"system","schema"}, configDirectories = {"conf","services"})
public class RadAuthServiceConfigurable extends RadiusServiceConfigurable {
	
	private static final String MODULE = "RAD_AUTH_CONFIGURABLE";
	...
	private String serviceAddress = "0.0.0.0:1812";
	...
	public RadAuthServiceConfigurable(){
	...
	@XmlElement(name = "service-address",type = String.class,defaultValue ="0.0.0.0:1812")
	...
	@XmlElement(name = "rrd")
	public RRDDetail getRRDDetails() {
		return rrdDetail;
	}
	public void setRRDDetails(RRDDetail rrdDetailsList ){
	...
	private void setPluginsCallerId(List<PluginEntryDetail> plugins, PluginMode mode) {
		for (int i = 0; i < plugins.size(); i++) {
			...
			PluginCallerIdentity key = PluginCallerIdentity.createAndGetIdentity(ServiceTypeConstants.RAD_AUTH, mode, i, data.getPluginName()).getId();
