 ...
 *
 */
public class AuthChargingGatewayCommunicationHandler extends ChargingGatewayCommunicationHandler<RadAuthRequest, RadAuthResponse> implements RadAuthServiceHandler {
	private static final String MODULE = "AUTH-CG-COMM-HNDLR";

	public AuthChargingGatewayCommunicationHandler(RadAuthServiceContext serviceContext, ExternalCommunicationEntryData data) {
	...
	@Override
	protected void handlePreRequest(RadAuthRequest request, RadAuthResponse response) {
	...
	class ResponseReceivedExecutor implements AsyncRequestExecutor<RadAuthRequest, RadAuthResponse> {
		...
		private RadUDPRequest remoteRequest;
		private RadUDPResponse remoteResponse;
	...
	class RequestDroppedExecutor implements AsyncRequestExecutor<RadAuthRequest, RadAuthResponse> {
		...
		@Override
		public void handleServiceRequest(RadAuthRequest serviceRequest, RadAuthResponse serviceResponse) {
			RadiusProcessHelper.rejectResponse(serviceResponse, AuthReplyMessageConstant.OPERATION_FAILED_FOR_CG_COMMUNICATION);
		}
	}
	...
	class RequestTimeoutExecutor implements AsyncRequestExecutor<RadAuthRequest, RadAuthResponse> {
		...
		public void handleServiceRequest(RadAuthRequest serviceRequest, RadAuthResponse serviceResponse) {
			if(getData().isAcceptOnTimeout() == false){
				RadiusProcessHelper.rejectResponse(serviceResponse, AuthReplyMessageConstant.TIMEOUT_RESPONSE_RECEIVED_FOR_CG_COMMUNICATION);
			} else {
				if (LogManager.getLogger().isLogLevel(LogLevel.INFO)) {
