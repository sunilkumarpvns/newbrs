...
public class AuthenticateOnlyRequestHandler implements WimaxRequestHandler {

	private static final String MODULE = "AUTH-ONLY_REQ_HANDLER";
	private KeyManager keyManager;
	private SPIKeyConfiguration spiKeyConfiguration;
	
	public AuthenticateOnlyRequestHandler(KeyManager keyManager, SPIKeyConfiguration spiKeyConfiguration){
	...
	public void handleRequest(WimaxRequest wimaxRequest, WimaxResponse wimaxResponse,
			WimaxSessionData wimaxSessionData) throws AuthorizationFailedException {
		if(LogManager.getLogger().isLogLevel(LogLevel.INFO))
			LogManager.getLogger().info(MODULE, "Handling Authenticate Only Request.");	
		...
		if(str_dhcp_ip_address != null){
			...
			if(LogManager.getLogger().isLogLevel(LogLevel.INFO)){
				LogManager.getLogger().info(MODULE, "No DHCP-IP-Address configured for the client so no keys related to DHCP will be added");
			}
		}
		...
		if(LogManager.getLogger().isLogLevel(LogLevel.TRACE))
			LogManager.getLogger().trace(MODULE, "Response packet after processing Authenticate-Only request: " + wimaxResponse);
	...
	public boolean isEligible(WimaxRequest wimaxRequest, WimaxResponse wimaxResponse) {
		...
		return (SerciceType.isAuthenticateOnly(serviceType));
	}
...
//		 FA_RK_SPI (value equals MN_HA_CMIP4_SPI)
	...
	protected void createAndSetDHCP_KEYS_AND_CONTEXT(WimaxSessionData wimaxSessionData, WimaxRequest wimaxRequest,WimaxResponse wimaxResponse, String str_dhcp_ip_address) throws AuthorizationFailedException {
		...
		long currentTimeInMillis = wimaxRequest.getRequestReceivedTimeInMillis();					
		try {
			dhcpKeyDetails = keyManager.getOrCreateDhcpKeyDetails(dhcp_ip_address,currentTimeInMillis);
		} catch (Exception e) {
			if (LogManager.getLogger().isInfoLogLevel()) {
		...
		((WimaxSessionDataImpl)wimaxSessionData).setDhcp_rk_lifetime_in_seconds(dhcpKeyDetails.getRemainingLifetimeInSeconds(currentTimeInMillis));
