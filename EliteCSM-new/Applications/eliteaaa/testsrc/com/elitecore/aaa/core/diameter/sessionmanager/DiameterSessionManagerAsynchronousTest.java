...
public class DiameterSessionManagerAsynchronousTest {
	...
	private IntervalBasedTask task;
	private TaskScheduler taskScheduler = new TaskScheduler() {

		@Override
		public Future<?> scheduleSingleExecutionTask(
		...
		public Future<?> scheduleIntervalBasedTask(IntervalBasedTask task) {
			DiameterSessionManagerAsynchronousTest.this.task = task;
			return null;
		}
	...
	public void before() throws Exception {		
		URL url = DiameterSessionManagerAsynchronousTest.class.getClassLoader().getResource("session-manager-asynchronous.xml");
		...
		Mockito.when(serverContext.getTaskScheduler()).thenReturn(taskScheduler);
...
//	}
	...
	public void testUpdate_ShouldUpdateData_IfExpressionIsSatisfied(String...strings) throws Exception {
		...
		try {
			...
			for(String string : strings) {
				String query = "INSERT INTO " + TABLE_NAME + " ( param1, param2, param3) VALUES ('" + string + "', '" + string + "', '" + string + "')";
...
//	}
	...
	private void checkForBatchModeAndExecute() throws InterruptedException {
		if ( configurable.getBatchParameters().isEnabled() == false) {
			return;
		}
		task.execute(Mockito.mock(AsyncTaskContext.class));
	}
	
	@Test(expected=NullPointerException.class)
	public void testInit_ShouldThrowNullPointerException_WhenBatchParametersAreNull() throws InitializationFailedException {
		configurable.setBatchParameters(null);
		configurable.postReadProcessing();
		DiameterSessionManager diaSessionManager = new DiameterSessionManager(serverContext, configurable);
		diaSessionManager.init();
	}
	...
	@Test
	public void byDefaultCreatesAnActiveSession() throws InitializationFailedException, SQLException, InterruptedException {
	...
	@Test
	public void usesValueOfSessionStatusAvpIfPresentInRequest() throws InitializationFailedException, SQLException, InterruptedException {
		DiameterSessionManager diameterSessionManager = new DiaSessionManagerStub(serverContext, configurable);
		DiameterRequest diameterRequest = new DiameterRequest();
		DiameterAnswer diameterAnswer = new DiameterAnswer();
		setUpRequest(diameterRequest, diameterAnswer );
