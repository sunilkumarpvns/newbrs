 ...
 */
public class RMSystemAlertManager extends SystemAlertManager implements IStackAlertManager{
	private static final String MODULE = "RM-SYSTEM-ALERT-MANAGER";
	private BaseAlertConfigurable alertManagerConfiguration;
	...
	private static final String DFLT_ALERT_FILE = "eliteaaa-rm-alert.log";

	

	public RMSystemAlertManager(AAAServerContext context) {
		super(context);		
	...
	public void initSevice() throws InitializationFailedException{
		...
		if(aaaServerConfiguration != null){		
			this.alertManagerConfiguration = aaaServerConfiguration.getAAAAlertManagerConfiguration();
			AlertListnersDetail alertListnersDetail = ((RMServerConfigurationImpl)((AAAServerContext)getServerContext()).getServerConfiguration()).getAlertListenersDetail();
			...
			if(alertListnersDetail != null){
				...
				if(configuredAlertListener!=null){
					initFileProcessors(alertManagerConfiguration.getFileTypeAlertsList(),configuredAlertListener);
					initSNMPTrapProcessors(alertManagerConfiguration.getSnmpTrapList(),configuredAlertListener);
				}
			}
	...
	private void initSNMPTrapProcessors(SNMPTraps snmpTypeAlerts,List<String> configuredAlertListener) {
	...
	public void scheduleAlert(StackAlertSeverity severity,
		...
		if (aaaAlertRelation == null) {
			LogManager.getLogger().warn(MODULE, "RM Alert Relation not defined for Diameter Alert: " + alert.id());
			return;
		}
		Alerts aaaAlert = Alerts.fromAlertId(aaaAlertRelation.aaaDiameterAlertId);
		if (aaaAlert == null) {
			LogManager.getLogger().warn(MODULE, "RM Alert for Diameter Alert: " + aaaAlertRelation.diameterAAAAlertId()+ 
					" not defined");
			return;
		}
