...
@Results({
        @Result(name= SUCCESS, type= RestGenericCTRL.REDIRECT_ACTION ,params = {NVSMXCommonConstants.ACTION_NAME,"diameter-packet-mapping"}),
})
public class DiameterPacketMappingCTRL extends RestGenericCTRL<PacketMappingData> {
    ...
    public HttpHeaders index() {
        ...
        List<PacketMappingData> diameterPacketMappings = Collectionz.newArrayList();
        packetMappings.stream().forEach(packetMappingData -> {
            if(ProtocolType.DIAMETER.getProtocolType().equals(packetMappingData.getCommProtocol())) {
                diameterPacketMappings.add(packetMappingData);
            }
        });
        setList(diameterPacketMappings);
    ...
    public HttpHeaders create() {
        ...
        packetMapping.setCommProtocol(ProtocolType.DIAMETER.name());
    ...
    public HttpHeaders update() {
        ...
        if (StringUtils.isEmpty(packetMapping.getId())) {
            getLogger().error(getLogModule(), "Error while updating Diameter Packet Mapping.Reason: Diameter Packet Mapping Id Not found");
        ...
        if (Objects.isNull(packetMappingFromDB)) {
            getLogger().error(getLogModule(), "Error while updating Diameter Packet Mapping.Reason: Diameter Packet Mapping not found with given Id: " + packetMapping.getId());
        ...
        packetMapping.setCommProtocol(ProtocolType.DIAMETER.name());
        packetMapping.getAttributeMappingData().setPacketMappingData(packetMapping);
        packetMapping.setDiameterGatewayProfilePacketMappings(packetMappingFromDB.getDiameterGatewayProfilePacketMappings());
        return super.update();
    }

    @Override
    protected boolean prepareAndValidateDestroy(PacketMappingData packetMapping) {
        if (Collectionz.isNullOrEmpty(packetMapping.getDiameterGatewayProfilePacketMappings()) == false) {
            addActionError("Packet Mapping " + packetMapping.getName() + " is associated with Diameter Gateway Profiles");
            String attachedGatewayProfiles = Strings.join(",", packetMapping.getDiameterGatewayProfilePacketMappings(), diameterGwProfilePacketMapData -> diameterGwProfilePacketMapData.getDiameterGatewayProfileData().getName());
            getLogger().error(getLogModule(), "Error while deleting packet mapping "+ packetMapping.getName() + ".Reason: packet mapping  is associated with "+ attachedGatewayProfiles +" Diameter Gateway Profiles");
            return false;
        }
        return true;
    }
    ...
    public void validate(){
        ...
        List<PacketType> packetTypes = PacketType.getPacketTypeList(ProtocolType.DIAMETER, ConversionType.valueOf(packetMappingData.getType()));
    ...
    public String getDiameterPacketMappingListAsJson(){
