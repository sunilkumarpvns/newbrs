...
<div class="panel panel-primary">
    <div class="panel-heading">
        <h3 class="panel-title"><s:text name="packet.mapping.update" /></h3>
    </div>
    <div class="panel-body">
        <s:form namespace="/sm/packetmapping" id="diameterPacketMappingUpdate" action="diameter-packet-mapping" method="post" cssClass="form-horizontal" validate="true" labelCssClass="col-xs-12 col-sm-4 col-lg-3" elementCssClass="col-xs-12 col-sm-8 col-lg-9" validator="validateForm()">
            <s:token/>
            ...
            <div class="row">
                <div class="col-xs-6">
                    ...
                    <s:textfield name="type" key="packet.mapping.conversationtype"
                              cssClass="form-control" id="conversationType"
                              tabindex="3" readonly="true" />

                    <s:select name="packetType" key="packet.mapping.packettype"
                              cssClass="form-control" list="@com.elitecore.corenetvertex.constants.PacketType@getPacketTypeList('DIAMETER')"
                              tabindex="4" id="packetType" />
                    <div id="attributeMappings">
                        <s:iterator value="attributeMappingData.mappings" status="index" var="mapping">
                            <s:if test="%{#mapping != null}" >
                            <s:hidden name="attributeMappingData.mappings[%{#index.count - 1}]"
                                      value="%{#mapping}"/>
                            </s:if>
                        </s:iterator>
                    </div>
            ...
            <div class="row">
                <div class="col-xs-12" align="center">
                    <button class="btn btn-sm btn-primary" type="submit" role="button"
                            formaction="${pageContext.request.contextPath}/sm/packetmapping/diameter-packet-mapping/${id}"><span
                    ...
                    <button type="button" id="btnCancel" class="btn btn-primary btn-sm" value="Cancel" tabindex="15"
                            onclick="javascript:location.href='${pageContext.request.contextPath}/sm/packetmapping/diameter-packet-mapping/${id}'">
...
<script type="text/javascript">
    $(function(){
        getPacketTypeBasedOnDiameterProtocolAndConversationType();
    ...
    function validateForm() {
        ...
        if (isValid == true) {
            isValid = verifyUniquenessOnSubmit('name', 'update', '<s:text name="id"/>', 'com.elitecore.corenetvertex.sm.gateway.PacketMappingData', '', '');
            if(isValid == true){
