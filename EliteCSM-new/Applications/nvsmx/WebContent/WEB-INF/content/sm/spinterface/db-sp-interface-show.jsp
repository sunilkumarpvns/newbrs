        ...
        <div class="nv-btn-group" align="right">
			<span class="btn-group btn-group-xs">
				<button type="button" class="btn btn-default header-btn" data-toggle="tooltip" data-placement="bottom" title="Audit History"
                        onclick="javascript:location.href='${pageContext.request.contextPath}/sm/audit/audit/${id}?auditableResourceName=${name}&refererUrl=/sm/spinterface/db-sp-interface/${id}'">
                    <span class="glyphicon glyphicon-eye-open" ></span>
                </button>
			</span>
            <span class="btn-group btn-group-xs">
				<button type="button" class="btn btn-default header-btn" data-toggle="tooltip" data-placement="bottom" title="edit"
                        onclick="javascript:location.href='${pageContext.request.contextPath}/sm/spinterface/db-sp-interface/${id}/edit'">
                    <span class="glyphicon glyphicon-pencil"></span>
                </button>
			</span>
            <span class="btn-group btn-group-xs" data-toggle="confirmation-singleton" onmousedown="deleteConfirm()" data-href="${pageContext.request.contextPath}/sm/spinterface/db-sp-interface/${id}?_method=DELETE">
                ...
                <div class="row">
                    <div class="col-sm-8">
                        <div class="row">
                            ...
                            <s:hrefLabel key="sp.interface.db.database" value="%{dbSpInterfaceData.databaseData.name}" cssClass="control-label light-text" labelCssClass="col-xs-5" elementCssClass="col-xs-7"
                                     url="/sm/database/database/%{dbSpInterfaceData.databaseData.id}" />
                            <s:label key="sp.interface.db.tablename" value="%{dbSpInterfaceData.tableName}" cssClass="control-label light-text" labelCssClass="col-xs-5" elementCssClass="col-xs-7" />
                            <s:label key="sp.interface.db.identityfield" value="%{dbSpInterfaceData.identityField}" cssClass="control-label light-text" labelCssClass="col-xs-5" elementCssClass="col-xs-7" />
                            <s:label key="sp.interface.db.maxquerytimeoutcount" value="%{dbSpInterfaceData.maxQueryTimeoutCount}" cssClass="control-label light-text" labelCssClass="col-xs-5" elementCssClass="col-xs-7" />
                        </div>
                    </div>
