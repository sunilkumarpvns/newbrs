	...
	<div class="panel-heading">
		<h3 class="panel-title">
			<s:if test="%{fupLevel == 0}">
				<s:text name="Update "/><s:text name="bod.qos.multiplier.hsqIPCANSessionMultiplier" />
			</s:if>
			<s:elseif test="%{fupLevel == 1}">
				<s:text name="Update "/><s:text name="bod.qos.multiplier.fup1IPCANSessionMultiplier"></s:text>
			</s:elseif>
			<s:elseif test="%{fupLevel == 2}">
				<s:text name="Update "/><s:text name="bod.qos.multiplier.fup2IPCANSessionMultiplier"></s:text>
			</s:elseif>
		</h3>
	</div>
		...
		<s:form namespace="/pd/bodqosmultiplier" action="bod-qos-multiplier" id="bodQosMultiplierCreate" method="post" cssClass="form-horizontal" validate="true" validator="validateForm()">
					...
					<div class="row">
						<div class="col-xs-12">
							<table id="serviceMultiplierTable" class="table table-blue table-bordered">
								...
								<tbody>
								<s:iterator value="bodServiceMultiplierDatas" status="i">
									<tr>
										<td style="width: 50%"><s:select list="serviceTypeDatas" listValue="name"
																		 value="%{serviceTypeData.id}" listKey="id"
																		 cssClass="form-control"
																		 name="bodServiceMultiplierDatas[%{#i.count - 1}].serviceTypeData.id"
																		 elementCssClass="col-xs-12"/>
										</td>
										<td style="width: 50%"><s:textfield cssClass="form-control"
																			value="%{multiplier}"
																			elementCssClass="col-xs-12" type="number"
																			name="bodServiceMultiplierDatas[%{#i.count - 1}].multiplier"
																			min="1.00" max="10.00" step="0.01" />
										</td>
			...
			<div class="row">
				<div class="col-xs-12" align="center">
					<button class="btn btn-sm btn-primary" type="submit" role="button" formaction="${pageContext.request.contextPath}/pd/bodqosmultiplier/bod-qos-multiplier/${id}" tabindex="22"><span class="glyphicon glyphicon-floppy-disk"></span> <s:text name="button.save"/></button>
...
<script>
var i = document.getElementById("serviceMultiplierTable").rows.length-1;
