...
<div class="panel panel-primary">
	...
	<div class="panel-body">
		<s:form namespace="/" action="policydesigner/qos/QosProfile/update" validate="true" id="qosProfileCreate" method="post" cssClass="form-horizontal" labelCssClass="col-xs-4 col-lg-2 text-right" elementCssClass="col-xs-8 col-lg-10" validator="validateForm()">
					...
					<div class="row">
						<div class="col-sm-8">
							...
							<s:if test="%{@com.elitecore.corenetvertex.constants.QuotaProfileType@RnC_BASED.name() == qosProfile.pkgData.quotaProfileType.name()}">
								...
								<s:if test="qosProfile.rateCardId == null && qosProfile.quotaProfileId == null ">
								...
								<s:elseif test="qosProfile.rateCardId == null" >
									<s:select 		name="qosProfile.quotaProfileId" key="qosprofile.attached.quotaprofile"  list="%{quotaProfileList}" listValue="name" listKey="id" id="quotaProfile" cssClass="form-control" headerKey="" headerValue="   SELECT " onchange="validateChargingType('quotaProfile','dataRateCardData')" />
									<s:select 		name="qosProfile.rateCardId" key="qosprofile.ratecard"  list="%{dataRateCardList}" listValue="name" listKey="id"  id="dataRateCardData" cssClass="form-control" headerKey="" headerValue="   SELECT " disabled="true"/>
								</s:elseif>
								<s:elseif test="qosProfile.quotaProfileId == null" >
									<s:select 		name="qosProfile.quotaProfileId" key="qosprofile.attached.quotaprofile"  list="%{quotaProfileList}" listValue="name" listKey="id" id="quotaProfile" cssClass="form-control" headerKey="" headerValue="   SELECT " disabled="true" />
									<s:select 		name="qosProfile.rateCardId" key="qosprofile.ratecard"  list="%{dataRateCardList}" listValue="name" listKey="id"  id="dataRateCardData" cssClass="form-control" headerKey="" headerValue="   SELECT " onchange="validateChargingType('dataRateCardData','quotaProfile')" />
								</s:elseif>
		 ...
		 <div class="row">
				<div class="col-xs-12" align="center">
					...
					<button id="btnBack" class="btn btn-primary btn-sm" type="button" value="Cancel" onclick="javascript:location.href='${pageContext.request.contextPath}/policydesigner/qos/QosProfile/view?qosProfileId=${qosProfile.id}'"><span class="glyphicon glyphicon-backward" title="Back"></span> <s:text name="button.qosprofile"></s:text> </button>
					<button id="btnBack" class="btn btn-primary btn-sm" type="button" value="Cancel" onclick="javascript:location.href='${pageContext.request.contextPath}/policydesigner/pkg/Pkg/view?pkgId=${qosProfile.pkgData.id}'"><span class="glyphicon glyphicon-backward" title="Back"></span> <s:text name="button.pkg"></s:text> </button>
				</div>
			</div>
		</s:form>
	</div>
...
function validateForm(){
    var isValidName = verifyUniquenessOnSubmit('qosProfileName','update','<s:text name="qosProfile.id" />','com.elitecore.corenetvertex.pkg.qos.QosProfileData','<s:text name="pkgId" />','');
...
function validateChargingType(obj1, obj2) {
    ...
    if(isNullOrEmpty(valueObj1) == false && isNullOrEmpty(valueObj2) == false) {
        setError(obj2,'<s:text name="qosprofile.validate.charging.type"/>');
        $("#"+obj2).val("");
    }
}
</script>
