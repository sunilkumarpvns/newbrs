...
<script type="text/javascript">
    ...
    <#list parameters.tagNames as tagName>
        ...
        if (form.elements['${aValidator.fieldName}']) {
            ...
            <#elseif validator.validatorType = "greaterThanZero">
            if (continueValidation && fieldValue != null && parseInt(fieldValue)<= 0) {
                setError(field.id, error);
                errors = true;
               	<#if validator.shortCircuit>continueValidation = false;</#if>
            ...
            <#elseif validator.validatorType = "email">
              <#-- to make validation case insensitive we use 'i' this is bug in struts 2 email validation which will be fixed in version 2.5
               we have directly use changes refer https://issues.apache.org/jira/browse/WW-4565 -->
            ...
            if (continueValidation && fieldValue != null) {
                if (<#if validator.min??>parseFloat(fieldValue) <
                     ${validator.min?c}<#else>false</#if> ||
                        <#if validator.max??>parseFloat(fieldValue) >
                           ${validator.max?c}<#else>false</#if>) {
                    setError(field.id, error);
                    errors = true;
                    <#if validator.shortCircuit>continueValidation = false;</#if>
            ...
            <#elseif validator.validatorType = "dateFormat">
            if (continueValidation && fieldValue != null && fieldValue.length > 0 && new Date(fieldValue) == "Invalid Date") {
                setError(field.id, error);
                errors = true;
                <#if validator.shortCircuit>continueValidation = false;</#if>
            }
            <#elseif validator.validatorType = "decimal">
            if (continueValidation && fieldValue != null) {
                var isDecimalValue = fieldValue.indexOf(".");
                if(isDecimalValue != -1){
	                var valueParts = fieldValue.split(".");
	        		if(valueParts[1].length > ${validator.allowDecimal?c} || valueParts.length > 2){
	        			setError(field.id, error);
                    	errors = true;
                    }
                }
