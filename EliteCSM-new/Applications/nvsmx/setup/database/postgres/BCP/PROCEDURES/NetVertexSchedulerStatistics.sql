...
CREATE OR REPLACE FUNCTION pkg_pcrf_scheduler_kpi_sp_scheduler_general ( CNT bigint, SUC bigint, INST bigint, J_NAME text, E_TIME bigint, RES_T bigint ) RETURNS VOID AS $body$
DECLARE
...
BEGIN
    ...
    SELECT pkg_pcrf_hist_func_check_table('TBLT_NOTIFICATION_HISTORY')
    INTO T_CNT;
    IF T_CNT    = 0 THEN
    ...
    ELSE
      FOR REC IN (SELECT
        ...
        NOTIFICATIONID
      FROM TBLT_NOTIFICATION_HISTORY B
      WHERE B.EMAILSTATUS != 'PENDING'
      AND SMSSTATUS       != 'PENDING'
      )
      LOOP
        CNT:=CNT+1;
        INSERT INTO  TBLT_NOTIFICATION_HISTORY
        SELECT *
        FROM TBLT_NOTIFICATION_QUEUE I
        WHERE I.NOTIFICATIONID = REC.NOTIFICATIONID;
        DELETE
        FROM TBLT_NOTIFICATION_QUEUE D
...
CREATE OR REPLACE FUNCTION pkg_pcrf_hist_sp_purge_notificationhist () 
RETURNS VOID 
LANGUAGE PLPGSQL
AS $body$
DECLARE
...
BEGIN
	...
	*/
    SELECT EXTRACT(EPOCH FROM current_time) into STIME;
      ...
      --ETIME     := DBMS_UTILITY.GET_TIME;
      TME_TAKEN := ETIME - STIME;
      perform pkg_pcrf_scheduler_kpi_sp_scheduler_general(CNT,SUC,INST,'JOB_SP_PURGE_NOTIFICATIONHIST',TME_TAKEN,T_CNT);
    --END IF;
  END;
$body$;
...
BEGIN
    SELECT EXTRACT(EPOCH FROM current_time) into STIME;
    SELECT pkg_pcrf_scheduler_kpi_func_check_partition('TBLT_SUBSCRIPTION_HISTORY')
    INTO P_CNT;
    IF P_CNT     = 0 THEN
      SELECT EXTRACT(EPOCH FROM current_time) into ETIME;
      --ETIME     := DBMS_UTILITY.GET_TIME;
      TME_TAKEN := ETIME - STIME;
      perform pkg_pcrf_scheduler_kpi_sp_scheduler_general(CNT,SUC,INST,'JOB_SP_PURGE_SUBSCRIPTION_HIST',TME_TAKEN,P_CNT);
    ELSE
      SELECT COUNT(*)
      INTO PRE_CNT
      FROM USER_TAB_PARTITIONS
      WHERE TABLE_NAME ='TBLT_SUBSCRIPTION_HISTORY';
...
-- ELITECSM NETVERTEX MODULE  JIRA : NETVERTEX-2028 
...
INSERT INTO pgagent.pga_jobstep (
    ...
    jid, 'JOB_SCHEDULER_EXECUTION_CHECK-Step1'::text, true, 's'::character(1),
...
/* ELITECSM NETVERTEX MODULE
  JIRA : NETVERTEX-2028 */
/*  
CREATE OR REPLACE FUNCTION pkg_pcrf_hist_sp_purge_tblm_audit () RETURNS VOID AS $body$
DECLARE
    CNT              bigint := 0;
    SUC              bigint := 0;
    STIME            bigint; -- := DBMS_UTILITY.GET_TIME;
...
BEGIN
    SELECT EXTRACT(EPOCH FROM current_time) into STIME;
    SELECT PKG_PCRF_SCHEDULER_KPI_FUNC_CHECK_PARTITION('TBLM_AUDIT')
    INTO P_CNT;
    IF P_CNT    = 0 THEN
      SELECT EXTRACT(EPOCH FROM current_time) into ETIME;
      --ETIME    := DBMS_UTILITY.GET_TIME;
      TME_TAKEN:=ROUND((ETIME - STIME)/100,1);
      PKG_PCRF_SCHEDULER_KPI_SP_SCHEDULER_GENERAL(CNT,SUC,INST,'JOB_PURGE_TBLM_AUDIT',TME_TAKEN,P_CNT);
    ELSE
	SELECT COUNT(*)
      INTO PRE_CNT
      FROM USER_TAB_PARTITIONS
      WHERE TABLE_NAME ='TBLM_AUDIT';
      FOR REC IN (SELECT TABLE_NAME,
        PARTITION_NAME
      FROM USER_TAB_PARTITIONS
      WHERE TABLE_NAME                                                     = 'TBLM_AUDIT'
...
DECLARE
    ...
    --ELITECSM NETEVRTEX MODULE JIRA : NETVERTEX-2028 
    CNT              bigint := 0;
    SUC              bigint := 0;
    STIME            bigint; -- := DBMS_UTILITY.GET_TIME;
    ETIME            bigint;
...
BEGIN
    SELECT EXTRACT(EPOCH FROM current_time) into STIME;
    SELECT PKG_PCRF_SCHEDULER_KPI_FUNC_CHECK_PARTITION('TBLT_USAGE_HISTORY')
    INTO P_CNT;
    IF P_CNT     = 0 THEN
      ETIME     := DBMS_UTILITY.GET_TIME;
      TME_TAKEN := ETIME - STIME;
    ...
    ELSE
      ...
      INTO PRE_CNT
      FROM USER_TAB_PARTITIONS
      WHERE TABLE_NAME ='TBLT_USAGE_HISTORY';
      FOR rec IN (SELECT table_name,
        partition_name
      FROM user_tab_partitions
      WHERE table_name                                                     = 'TBLT_USAGE_HISTORY'
