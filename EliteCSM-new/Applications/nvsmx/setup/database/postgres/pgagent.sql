...
CREATE TABLE IF NOT EXISTS pgagent.pga_jobagent (
...
CREATE TABLE IF NOT EXISTS pgagent.pga_jobclass (
jclid                serial               NOT NULL PRIMARY KEY,
jclname              text                 NOT NULL
) WITHOUT OIDS;
CREATE UNIQUE INDEX IF NOT EXISTS pga_jobclass_name ON pgagent.pga_jobclass(jclname);
...
CREATE TABLE IF NOT EXISTS pgagent.pga_job (
...
CREATE TABLE IF NOT EXISTS pgagent.pga_jobstep (
...
jstkind              char                 NOT NULL CHECK (jstkind IN ('b', 's')), 
...
jstonerror           char                 NOT NULL CHECK (jstonerror IN ('f', 's', 'i')) DEFAULT 'f',
jscnextrun           timestamptz          NULL
) WITHOUT OIDS;

CREATE INDEX IF NOT EXISTS pga_jobstep_jobid ON pgagent.pga_jobstep(jstjobid);
...
CREATE TABLE IF NOT EXISTS pgagent.pga_schedule (
...
CREATE INDEX IF NOT EXISTS pga_jobschedule_jobid ON pgagent.pga_schedule(jscjobid);
COMMENT ON TABLE pgagent.pga_schedule IS 'Schedule for a job';



CREATE TABLE IF NOT EXISTS pgagent.pga_exception (
...
WITHOUT OIDS;
CREATE INDEX IF NOT EXISTS pga_exception_jexscid ON pgagent.pga_exception (jexscid);
CREATE UNIQUE INDEX IF NOT EXISTS pga_exception_datetime ON pgagent.pga_exception (jexdate, jextime);
COMMENT ON TABLE pgagent.pga_schedule IS 'Job schedule exceptions';



CREATE TABLE IF NOT EXISTS pgagent.pga_joblog (
...
CREATE INDEX IF NOT EXISTS pga_joblog_jobid ON pgagent.pga_joblog(jlgjobid);
...
CREATE TABLE IF NOT EXISTS pgagent.pga_jobsteplog (
...
CREATE INDEX IF NOT EXISTS pga_jobsteplog_jslid ON pgagent.pga_jobsteplog(jsljlgid);
...
CREATE OR REPLACE FUNCTION pgagent.pgagent_schema_version() RETURNS int2 LANGUAGE 'plpgsql' VOLATILE AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION  pgagent.pga_next_schedule(int4, timestamptz, timestamptz, _bool, _bool, _bool, _bool, _bool) RETURNS timestamptz LANGUAGE 'plpgsql' VOLATILE
AS $$
DECLARE
    jscid           ALIAS FOR $1;
...
CREATE OR REPLACE FUNCTION pgagent.pga_is_leap_year(int2) RETURNS bool LANGUAGE 'plpgsql' IMMUTABLE AS $$
BEGIN
    IF $1 % 4 != 0 THEN
        RETURN FALSE;
    END IF;
...
CREATE OR REPLACE FUNCTION  pgagent.pga_job_trigger() RETURNS "trigger" LANGUAGE 'plpgsql' VOLATILE AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION  pgagent.pga_schedule_trigger() RETURNS trigger LANGUAGE 'plpgsql' VOLATILE AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION  pgagent.pga_exception_trigger() RETURNS "trigger" LANGUAGE 'plpgsql' VOLATILE
AS $$
DECLARE
