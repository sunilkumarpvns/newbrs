...
AS
  ...
  BEGIN
    SELECT PKG_PCRF_SCHEDULER_KPI.FUNC_CHECK_PARTITION('TBLM_AUDIT')
    INTO P_CNT
    FROM DUAL;
    IF P_CNT    = 0 THEN
      ETIME    := DBMS_UTILITY.GET_TIME;
      TME_TAKEN:=ROUND((ETIME - STIME)/100,1);
      PKG_PCRF_SCHEDULER_KPI.SP_SCHEDULER_GENERAL (CNT,SUC,INST,'JOB_PURGE_TBLM_AUDIT',TME_TAKEN,P_CNT);
    ELSE
	SELECT COUNT(*)
      INTO PRE_CNT
      FROM USER_TAB_PARTITIONS
  ...
  BEGIN
    /*  EliteCSM - NetVertex Module
    JIRA : NETVERTEX-2028  */
    SELECT PKG_PCRF_HIST.FUNC_CHECK_TABLE('TBLT_NOTIFICATION_HISTORY')
    INTO T_CNT
    FROM DUAL;
    IF T_CNT    = 0 THEN
      ETIME    :=DBMS_UTILITY.GET_TIME;
    ...
    ELSE
       FOR REC IN
		(SELECT
			...
			ID
			FROM TBLT_NOTIFICATION_QUEUE B
			WHERE B.EMAIL_STATUS IN ('SENT','FAILED')
			AND B.SMS_STATUS     IN('SENT','FAILED')
		)
   LOOP
        CNT:=CNT+1;
  ...
  AS
    ...
    pragma exception_init(x_last_partition, -14758);
  BEGIN
    SELECT PKG_PCRF_SCHEDULER_KPI.FUNC_CHECK_PARTITION('TBLT_SUBSCRIPTION_HISTORY')
    INTO P_CNT
    FROM DUAL;
    IF P_CNT     = 0 THEN
      ETIME     := DBMS_UTILITY.GET_TIME;
  ...
  AS
    ...
    pragma exception_init(x_last_partition, -14758);
  BEGIN
    SELECT PKG_PCRF_SCHEDULER_KPI.FUNC_CHECK_PARTITION('TBLT_USAGE_HISTORY')
    INTO P_CNT
    FROM DUAL;
    IF P_CNT     = 0 THEN
      ETIME     := DBMS_UTILITY.GET_TIME;
...
--Scheduler for SP_PURGE_TBLM_AUDIT

--Creating new job
BEGIN
DBMS_SCHEDULER.CREATE_JOB (
   job_name           =>  'JOB_SP_PURGE_TBLM_AUDIT',
   job_type           =>  'STORED_PROCEDURE',
   job_action         =>  'PKG_PCRF_HIST.SP_PURGE_TBLM_AUDIT',
   start_date         =>  SYSTIMESTAMP,
