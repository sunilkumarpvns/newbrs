...
<script type="text/javascript">
	function subscribePromotionalOffer(addOnPackageID){
		...
		if(agree){
			document.forms[0].addOnPackageId.value=addOnPackageID;
	...
	function unSubscribePromotionalOffer(addOnPackageID){
		...
		var agree = confirm(msg);
		if(agree){
			document.forms[1].addOnPackageId.value=addOnPackageID;
...
<div class="border" >
	...
	<table width="97%" cellpadding="0" cellspacing="0" border="0" style="margin-right:10px;margin-left: 10px;margin-top: 10px;margin-bottom: 10px" >
		...
		<tr class="table-org" height="30px">   		
   			<td class="table-org-column" colspan="3"><bean:message key="parentalcontrol.prmoffer.availablepromo"/></td>    		
		</tr>
		...
		<html:hidden property="selectedPortal" value="parental"/>
		<tr>   		
   			<td colspan="3" >
   				<table width="100%" cellpadding="0" cellspacing="0" border="0">
   				<%if(promotionalOfferForm.getAvailableAddons() != null && promotionalOfferForm.getAvailableAddons().length>0){%>	
				<%int i=0;%>									
					<tr height="20px">
						...
						<logic:iterate id="addonData" name="promotionalForm" property="availableAddons" type="com.elitecore.netvertexsm.ws.xsd.PromotionalData">
							<%
								String addOnPackageId=""+addonData.getAddOnPackageId();
								boolean isSubscribed=false;
								if(promotionalOfferForm.getActicvePromotions() != null && promotionalOfferForm.getActicvePromotions().length>0){
							%>						
							<logic:iterate id="activePromotionalData" name="promotionalForm" property="acticvePromotions" type="com.elitecore.netvertexsm.ws.xsd.PromotionalData">
								<logic:equal value="<%=addOnPackageId%>" property="addOnPackageId" name="activePromotionalData">							
									<%isSubscribed=true;%>												 								
								</logic:equal>						
							</logic:iterate>
							<% 	} 
								if(!isSubscribed || promotionalOfferForm.getActicvePromotions() == null){	
							%>
			   				<tr class="<%=(++i%2==0)?"table-white":"table-gray"%>">
								<td valign="center" style="text-align: left;" class="table-column">
									<bean:write name="addonData" property="name" />&nbsp;
								</td>
								<td valign="center" class="table-column" style="text-align: center;">
									<bean:write name="addonData" property="price" />&nbsp;
								</td>			   				
								<td valign="center" class="table-column" style="text-align: center;">
									<bean:write name="addonData" property="validity" />&nbsp;<bean:write name="addonData" property="validityPeriod" />
								</td>			   				
								<td valign="center" class="table-column" style="text-align: center;">
									<logic:notEmpty name="addonData" property="offerEndDate">
										<logic:notEqual value="-" name="addonData" property="offerEndDate">
									<bean:write name="addonData" property="offerEndDate" />&nbsp;
										</logic:notEqual>										
									</logic:notEmpty>
								</td>
								<td valign="center" class="table-column" style="text-align: center;">
									<input type="button" name="Subscribe" class="orange-btn" value="  <bean:message key="parentalcontrol.bod.subscribe.button"/>  " onclick="subscribePromotionalOffer('<bean:write name="addonData" property="addOnPackageId" />');">&nbsp;&nbsp;
								</td>			   				
							</tr>
							<% 	}%>
						</logic:iterate>
						...
						<tr class="table-white">
							<td align="center" class="tblrows" colspan="5"><bean:message key="general.norecordsfound"/></td>				
						</tr>								
								<%
							}
						%>
			...
			<table width="100%" cellpadding="0" cellspacing="0" border="0" >
					...
					<tr><td colspan="6" class="smallgap" >&nbsp;</td></tr>
						...
						<tr height="20px">
							...
							<td class="table-org-column" align="center">Validity</td>
						...
						<td>
				    		...
				    		<c:forEach var="subscriptionHistoryData" begin="<%=begin%>" end="<%=end%>" items="<%=promotionalOfferForm.getSubscriptionHistoriesList()%>">
				    			...
				    			<tr class="<%=(i%2==0)?"table-white":"table-gray"%>">
				    				<td style="text-align: left;" class="table-column">${subscriptionHistoryData.addOnName}</td>
				    				<td style="text-align: center;" class="table-column">${subscriptionHistoryData.validityPeriod} ${subscriptionHistoryData.validityPeriodUnit}</td>
				    				<td style="text-align: center;" class="table-column">${subscriptionHistoryData.startDate}</td>
				    				<td style="text-align: center;" class="table-column">${subscriptionHistoryData.endDate}</td>
				    				<td style="text-align: center;" class="table-column">${subscriptionHistoryData.status}</td>
		...
		<tr><td colspan="3" class="smallgap" >&nbsp;</td></tr>
   		</table>
	</div>
	</div>
</body>