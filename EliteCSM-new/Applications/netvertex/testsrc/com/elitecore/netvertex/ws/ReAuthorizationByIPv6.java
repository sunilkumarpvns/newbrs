...
public class ReAuthorizationByIPv6 {

    private final PCRFKeyConstants pcrfKey = PCRFKeyConstants.CS_SESSION_IPV6;
    private final String ipv6 = "fe80::75aa:8b94:2c3f:2ee3";
    ...
    public void returnNotFoundWhenSessionNotFromDBAndCache() {
        when(sessionLocator.getCoreSessionByUserIdentityFromCache(ipv6)).thenReturn(null);
        when(sessionLocator.getCoreSessionList(any())).thenReturn(null);
        Assert.assertTrue(JMXConstant.SESSION_NOT_FOUND.equalsIgnoreCase(reAuthorizationController.reAuthBySessionIPv6(ipv6)));
    }
    ...
    public void reAuthorizeSessionFromCacheWhenSessionNotFoundFromDB() {
        ...
        firstSessionFromCache.addValue(pcrfKey.getVal(), ipv6);
        ...
        when(sessionLocator.getCoreSessionBySessionIPv6FromCache(ipv6)).thenReturn(sessionsFromCache.iterator());
    ...
    public void reAuthorizeSessionsFromDBWhenSessionNotFoundFromCache() {
        ...
        firstSessionFromDB.addValue(pcrfKey.getVal(), ipv6);
        ...
        secondSessionFromDB.addValue(PCRFKeyConstants.CS_SESSION_TYPE.getVal(), SessionTypeConstant.GY.val);
        sessionsFromDB.add(secondSessionFromDB);

        when(sessionLocator.getCoreSessionBySessionIPv6FromCache(ipv6)).thenReturn(null);
    ...
    public void reAuthorizeLatestSessionsFromDBWhenSessionFoundFromCacheAndDBBoth() {
        ...
        Date firstSessionCreationTimeInDB = calendar.getTime();
        SessionData firstSessionFromDB = createSessionData(firstSessionCreationTimeInDB, Arrays.asList(Maps.<PCRFKeyConstants, String>newLinkedHashMap(
                Arrays.asList(
                        ...
                        Maps.Entry.newEntry(PCRFKeyConstants.CS_SESSION_IPV4, ipv6)
        ...
        SessionData secondSessionFromDB = createSessionData(secondSessionCreationTimeInDB, Arrays.asList(Maps.<PCRFKeyConstants, String>newLinkedHashMap(
                Arrays.asList(
        ...
        Date firstSessionCreationTimeInCache = calendar.getTime();
        SessionData firstSessionFromCache = createSessionData(firstSessionCreationTimeInCache, Arrays.asList(Maps.<PCRFKeyConstants, String>newLinkedHashMap(
                Arrays.asList(
                        Maps.Entry.newEntry(PCRFKeyConstants.CS_CORESESSION_ID, firstSessionCSID),
                        Maps.Entry.newEntry(PCRFKeyConstants.CS_SESSION_TYPE, SessionTypeConstant.GX.val),
                        Maps.Entry.newEntry(PCRFKeyConstants.CS_SESSION_IPV4, ipv6)
        ...
        Date secondSessionCreationTimeInCache = calendar.getTime();
        SessionData secondSessionFromCache = createSessionData(secondSessionCreationTimeInCache, Arrays.asList(Maps.<PCRFKeyConstants, String>newLinkedHashMap(
                Arrays.asList(
                        Maps.Entry.newEntry(PCRFKeyConstants.CS_CORESESSION_ID, secondSessionCSID),
                        Maps.Entry.newEntry(PCRFKeyConstants.CS_SESSION_TYPE, SessionTypeConstant.GY.val)
    ...
    private void reAuthorizeSessionAssertion(SessionData session1, SessionData session2) {
        ...
        Assert.assertEquals(JMXConstant.SUCCESS,reAuthorizationController.reAuthBySessionIPv6(ipv6));
    ...
    private void prepareCriteria() throws SessionException {
        ...
        criteria.add(Restrictions.eq(pcrfKey.val, ipv6));
