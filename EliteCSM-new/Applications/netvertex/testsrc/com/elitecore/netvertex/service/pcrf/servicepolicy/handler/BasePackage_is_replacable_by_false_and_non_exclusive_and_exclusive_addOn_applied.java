...
@Ignore
public class BasePackage_is_replacable_by_false_and_non_exclusive_and_exclusive_addOn_applied {/*
	...
	public class SessionQoSSelection {
		...
		@Test
		public void test_process_should_select_non_exclusive_addOn_QoS_when_exclusive_addOn_not_applied_and_have_higher_qos_than_base_package() throws OperationFailedException {
			...
			AddOn exclusive2 = AddOnPackageFactory.createNonExclusiveAddOn("non-exclusive").hasHigherQosThan(basePackage).build();
		...
		@Test
		public void test_process_should_select_non_exclusive_addOn_QoS_when_exclusive_addOn_not_applied_and_have_equal_qos_than_base_package() throws OperationFailedException {
			...
			AddOn exclusive2 = AddOnPackageFactory.createNonExclusiveAddOn("non-exclusive").hasEqualQosThan(basePackage).build();
			AddOnSubscription exclusive2Subscription = SubscriptionFactory.createSubscriptionFor(exclusive2).build();
		...
		@Test
		public void test_process_should_select_base_package_QoS_when_exclusive_addOn_not_applied_and_have_higher_qos_than_non_exclusvie_addOn() throws OperationFailedException {
			...
			AddOn exclusive2 = AddOnPackageFactory.createNonExclusiveAddOn("non-exclusive").hasLowerQosThan(basePackage).build();
			AddOnSubscription exclusive2Subscription = SubscriptionFactory.createSubscriptionFor(exclusive2).build();
	...
	public class PCCRuleSelection {
		...
		@Test
		public void test_process_should_select_pccRules_of_non_exclusive_addOn_subscription_if_both_exclusive_non_exclusive_addOn_subscription_has_same_pccRule_qos() throws OperationFailedException {
		...
		@Test
		public void test_process_should_select_best_qos_pccRules_when_exclusive_addOn_not_applied() throws OperationFailedException {
			...
			//Non-Exclusive addOn
			PCCRule exclusiveAddOn2FtpPCCRule = PCCRuleFactory.createPCCRuleHasHigherQoSThan(ftpPCCRule);
			PCCRule exclusiveAddOn2HttpPCCRule = PCCRuleFactory.createPCCRuleHasLowerQoSThan(httpPCCRule);
			
			QuotaProfile exclusiveAddOnQuotaProfile2 = QuotaProfileFactory.createQuotaProfileWithRandomUsage(UUID.randomUUID().toString());
			QoSProfileDetail exclusiveAddOnQoSProfileDetail2 = QoSProfileDetailFactory.createQoSProfile().
					hasQuotaProfileDetail(exclusiveAddOnQuotaProfile2.getHsqLevelServiceWiseQuotaProfileDetails().get(CommonConstants.ALL_SERVICE_ID)).
					pccRules(Arrays.asList(exclusiveAddOn2FtpPCCRule,exclusiveAddOn2HttpPCCRule)).build();
			QoSProfile exclusiveAddQoSProfile2 = QosProfileFactory.createQosProfile().quotaProfileDetail(exclusiveAddOnQuotaProfile2).hasHSQLevelQoSProfileDetail(exclusiveAddOnQoSProfileDetail2).build();
			AddOn exclusiveAddOn2 = new AddOn.AddOnBuilder(UUID.randomUUID().toString(),"non-exclusive")
					.addQoSProfile(exclusiveAddQoSProfile2).wihtValidityPeriodUnit(ValidityPeriodUnit.DAY).build();
			
			
			AddOnSubscription addOnSubscription2 = SubscriptionFactory.createSubscriptionFor(exclusiveAddOn2).build();
			LinkedHashMap<String, AddOnSubscription> addOnSubscriptions = new LinkedHashMap<String, AddOnSubscription>();
			addOnSubscriptions.put(addOnSubscription2.getAddonSubscriptionId(), addOnSubscription2);
			addOnSubscriptions.put(addOnSubscription.getAddonSubscriptionId(), addOnSubscription);
			...
			List<PCCRule> expectedPCCRules = Arrays.asList(exclusiveAddOn2FtpPCCRule,httpPCCRule);
			getLogger().debug("", pcrfResponse.toString());
			assertReflectionEquals(expectedPCCRules, pcrfResponse.getActivePCCRules(), ReflectionComparatorMode.LENIENT_ORDER);
			assertReflectionEquals(expectedPCCRules, pcrfResponse.getInstallablePCCRules(), ReflectionComparatorMode.LENIENT_ORDER);
			assertNull(pcrfResponse.getRemovablePCCRules());
		...
		@Test
		public void test_process_should_select_base_package_pccRules_when_non_exclusive_addOn_and_exclusive_addOn_not_applied() throws OperationFailedException {
