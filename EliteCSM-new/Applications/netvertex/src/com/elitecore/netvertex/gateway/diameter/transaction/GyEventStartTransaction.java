...
public class GyEventStartTransaction extends BaseTransaction {

	private static Map<String, TransactionAction> actionHandlersMap;

	private static final String MODULE = "GY-EVNT-STRT-TRNSCT";

	static {
		actionHandlersMap = new HashMap<String, TransactionAction>(1, 1);
		actionHandlersMap.put(EventTypes.EVENT_REQUEST_RCVD, TransactionAction.GY_AUTHORIZE);
		actionHandlersMap.put(EventTypes.PCRF_RES_RCVD, TransactionAction.SEND_CCA);
	}

	public GyEventStartTransaction(DiameterGatewayControllerContext diameterGatewayControllerContext) {
		super(diameterGatewayControllerContext);
	}
	
	@Override
	public String getType() {
		return GyTransactionType.EVENT_REQUEST.name();
	}
	...
	public synchronized void process(String eventType, DiameterRequest request, DiameterAnswerListener diameterAnswerListener) {
		...
		if (EventTypes.EVENT_REQUEST_RCVD.equals(eventType) == false) {
			getLogger().error(MODULE, "Can't process " + getType() + " transaction. Reason: expected event = " + EventTypes.EVENT_REQUEST_RCVD
	...
	public Object clone() throws CloneNotSupportedException {
		return new GyEventStartTransaction(getControllerContext());
	}
