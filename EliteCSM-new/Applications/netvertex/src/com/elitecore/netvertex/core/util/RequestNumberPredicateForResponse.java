...
public class RequestNumberPredicateForResponse implements Predicate<PCRFResponse, SessionData>{

	private static final String MODULE = "REQ-NUM-PREDICATE-RES";
	private static RequestNumberPredicateForResponse requestNumberPredicate;
	
	static {
		requestNumberPredicate = new RequestNumberPredicateForResponse();
	}
	
	public static RequestNumberPredicateForResponse getInstance() {
		return requestNumberPredicate;
	}
	
	@Override
	public boolean apply(@Nonnull PCRFResponse pcrfResponse, SessionData sessionData) {
		
		if (sessionData == null) {
			return true;
		}
		
		String currentRequestNumber = pcrfResponse.getAttribute(PCRFKeyConstants.REQUEST_NUMBER.val);
