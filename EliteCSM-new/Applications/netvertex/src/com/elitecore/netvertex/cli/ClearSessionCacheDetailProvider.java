...
public class ClearSessionCacheDetailProvider<K,V> extends DetailProvider{

	private HashMap<String ,DetailProvider> detailProviderMap;
	@Nonnull private Cache<K, V> cache;
	private static final String HELP = "-help";
	private static final String ALL = "-all";
	private static final String IDLE = "-idle";
	private String key;

	public ClearSessionCacheDetailProvider(String key, Cache<K, V> primaryCache) {
		this.key = key;
	...
	public String execute(String[] parameters) {
		if (parameters.length > 0) {
			...
			if (ALL.equalsIgnoreCase(parameters[0])) {
				...
				return "Total " + count + " session cache flushed" + getCacheStatistics() ;
			}
	...
	private String getCacheStatistics() {
		...
		dataRecord[1] = "" + cache.statistics().getCacheCount();
		dataRecord[2] = "";
		dataRecord[3] = "";
		dataRecord[4] = "";
		dataRecord[5] = "";
		...
		dataRecord1[1] = "" + cache.statistics().getRequestCount();
		dataRecord1[2] = "Total-Cache-Hit      : ";
		dataRecord1[3] = "" + cache.statistics().getHitCount();
		dataRecord1[4] = "Total-Cache-Miss     : ";
		dataRecord1[5] = "" + cache.statistics().getMissCount();

		String[] dataRecord2 = new String[6];
		dataRecord2[0] = "Total-Load-Count    : ";
		dataRecord2[1] = "" + cache.statistics().getLoadCount();
		dataRecord2[2] = "Average-Load-Panelty : ";
		dataRecord2[3] = "" + cache.statistics().getAverageLoadPanelty();
		dataRecord2[4] = "Total-Eviction-Count : ";
		dataRecord2[5] = "" + cache.statistics().getEvictionCount();
	...
	public String getHelpMsg() {
		...
		out.println(" " + ALL + " : flush all session cache and statistics");
		out.println(" " + IDLE + " : flush idle session cache");
