...
public class VoltDBDetailProvider extends DetailProvider {
    private static final String DESCRIPTION = "List the details of DB datasource";
    private static final String DB = "-voltdb";
    ...
    private static final String[] headers = { "DB DATASOURCES",
            "STATUS" };
    private static final int[] width = { 26, 15 };
    private static final int[] columnAlignment = { TableFormatter.CENTER,
            TableFormatter.CENTER};

    public VoltDBDetailProvider( NetVertexServerContext serverContext) {
    ...
    private String getViewHelpMessage(){
        ...
        out.println("Description : Shows details of VoltDB datasource " );
    ...
    public String getHelpMsg() {
        ...
        out.println("Possible options:");
        out.println();
        out.println(EliteBaseCommand.fillChar("\t" + VIEW, 25) + "-Shows details of VoltDB datasource");
    ...
    public String dbFormat(Map<String, DBDataSource> dbDatasourceMapParameter) {
        ...
        for (DBDataSource dbDatasource : dbDatasourceMapParameter.values()) {
            ...
            List<String> dataSources = voltDBClientManager.getDataSources();
    ...
    private boolean isAlive(String dataSourceName) {
        return serverContext.getVoltDBClientManager().getVoltDBClient(dataSourceName).isAlive();
    }

    private boolean isInitialized(String dbDatasourceName) {
        VoltDBClient voltDBClient = serverContext.getVoltDBClientManager().getVoltDBClient(dbDatasourceName);

        return (voltDBClient == null)?false:true;
    }
    ...
    protected Map<String, DBDataSource> getDBDatasourceMap(){
        ...
        if (dataSource.getConnectionURL().contains(CommonConstants.VOLTDB) == true) {
