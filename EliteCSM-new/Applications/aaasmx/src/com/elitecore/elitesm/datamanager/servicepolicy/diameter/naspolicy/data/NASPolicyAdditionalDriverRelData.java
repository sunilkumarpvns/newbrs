...
@ValidObject
@XmlRootElement(name = "additional-driver-detail")
public class NASPolicyAdditionalDriverRelData  extends BaseData implements Serializable, Validator {
		...
		if (Strings.isNullOrBlank(this.driverName) == false) {
			...
			try {
				...
				if ((ServiceTypeConstants.NAS_AUTH_APPLICATION == driverTypeData.getServiceTypeId()) == false) {
					RestUtitlity.setValidationMessage(context, "Specify driver: " + getDriverName() + " in additional group is not Auth Type driver");
					isValid = false;
				} else {
