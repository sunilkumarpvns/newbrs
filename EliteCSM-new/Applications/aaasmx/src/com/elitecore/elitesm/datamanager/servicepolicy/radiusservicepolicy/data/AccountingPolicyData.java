...
public class AccountingPolicyData implements Differentiable, Validator {
	...
	@Valid
	@Size(min = 1, message = "Specify atleast one handler in accouting flow")
	...
	public AcctPostResponseHandlerData getPostResponseHandlerData() {
		return postResponseHandlerData;
	}
	
	public void setPostResponseHandlerData(
			AcctPostResponseHandlerData postResponseHandlerData) {
		this.postResponseHandlerData = postResponseHandlerData;
	}
	
	@XmlTransient
	public String isValidatePacket() {
		return radiusServicePolicyData.getValidatePacket();
	}
	...
	public boolean validate(ConstraintValidatorContext context) {
		...
		if (Collectionz.isNullOrEmpty(prePlugins) == false) {
			...
			try {
				List<PluginInstData> authPluginList = blManager.getAcctPluginList();
				if (Collectionz.isNullOrEmpty(authPluginList) == false) {
					for (PluginEntryDetail prePlugin : prePlugins) {
						...
						if (Strings.isNullOrBlank(pluginName) == false) {
							...
							if (isValidPrePlugin == false) {
								RestUtitlity.setValidationMessage(context, "Plugin: " + pluginName + " in " + "Pre Plugin is not of Acct Type");
								isValid = false;
							}
						}
					}
					...
					isValid = false;
					RestUtitlity.setValidationMessage(context, "Acct Type of plugins does not available in system");
				}
			} catch (DataManagerException e) {
				e.printStackTrace();
				RestUtitlity.setValidationMessage(context, "Failed to retrieve Acct Type of plugins");
				isValid = false;
			}
		}
					...
					for (PluginEntryDetail postPlugin : postPlugins) {
						...
						if (Strings.isNullOrBlank(pluginName) == false) {
							...
							if (isValidPostPlugin == false) {
								RestUtitlity.setValidationMessage(context, "Plugin: " + pluginName + " in " + "Post Plugin is not of Acct Type");
								isValid = false;
							}
						}
					}
		...
		for (AcctServicePolicyHandlerData handler : getHandlersData()) {		
			
			if(handler instanceof RadiusSubscriberProfileRepositoryDetails){
				...
				boolean isValidPluginInPluginHandler = true;
				...
				RestUtitlity.setValidationMessage(context, "Authentication Handler is not supported in accounting flow");
				isValid = false;
			} else if (handler instanceof AuthorizationHandlerData) {
				RestUtitlity.setValidationMessage(context, "Authorization Handler is not supported in accounting flow");
				isValid = false;
			} else if (handler instanceof ConcurrencyHandlerData) {
				RestUtitlity.setValidationMessage(context, "Concurrency Handler is not supported in accounting flow");
				isValid = false;
			} 
		}
		...
		if (this.postResponseHandlerData != null) {
			...
			for (AcctServicePolicyHandlerData handler : getPostResponseHandlerData().getHandlersData()) {
				...
				if (handler instanceof RadPluginHandlerData) {
					...
					try {
						...
						List<PluginInstData> acctPluginList = blManager.getAcctPluginList();
								...
								if (Strings.isNullOrBlank(pluginName) == false) {
									...
									if (isValidPostPluginHandler == false) {
										RestUtitlity.setValidationMessage(context,  "Plugin: " + pluginName + 
												" in Post Response " + HandlerConstants.PLUGIN_HANDLER + " is not of Acct type");
										isValid = false;
									}
								}
							}
					...
					RestUtitlity.setValidationMessage(context, "Authentication Handler is not supported in post response of accounting flow");
					isValid = false;
				} else if (handler instanceof AuthorizationHandlerData) {
					RestUtitlity.setValidationMessage(context, "Authorization Handler is not supported in post response of accounting flow");
					isValid = false;
				} else if (handler instanceof ConcurrencyHandlerData) {
					RestUtitlity.setValidationMessage(context, "Concurrency Handler is not supported in post response of accounting flow");
					isValid = false;
				} else if (handler instanceof RadiusSubscriberProfileRepositoryDetails) {
					RestUtitlity.setValidationMessage(context, "Profile Look Up Handler is not supported in post response of accounting flow");
					isValid = false;
				} else if (handler instanceof SynchronousCommunicationHandlerData) {
					RestUtitlity.setValidationMessage(context, "Proxy(Serial) Handler is not supported in post response of accounting flow");
					isValid = false;
				} else if (handler instanceof BroadcastCommunicationHandlerData) {
					RestUtitlity.setValidationMessage(context, "Broadcast(Parellel) Handler is not supported in post response of accounting flow");
					isValid = false;
				}
			}
		}
			...
			if (externalCommunicationEntryData != null) {
				...
				if( Strings.isNullOrBlank(externalCommunicationEntryData.getAcceptOnTimeout())){
					RestUtitlity.setValidationMessage(context,
							"Accept On Timeout must be specified  in "+HandlerConstants.PROXY_HANDLER+" , It could be 'true' or 'false'");
					isValid = false;
				}else if(BOOLEAN_REGEX.matcher(externalCommunicationEntryData.getAcceptOnTimeout()).matches() == false){
					RestUtitlity.setValidationMessage(context,
							"Invalid value of Accept On Timeout [ " + externalCommunicationEntryData.getAcceptOnTimeout()+" ] in "+HandlerConstants.PROXY_HANDLER+" of Accounting Flow, It could be 'true' or 'false'");
					isValid = false;
				} 	
			...
			if (coaDMEntryData != null) {
				...
				if( Strings.isNullOrBlank(coaDMEntryData.getPacketType()) == false ){
					if ("COA Request".equals(coaDMEntryData.getPacketType()) == false
						...
						RestUtitlity.setValidationMessage(context, "Invalid Packet type is specified in COA/DM Handler of Accounting Flow. It could be 'COA Request' or 'Disconnect Request' ");
						isValid = false;
					}
				}else{
					RestUtitlity.setValidationMessage(context, "Packet Type must be specified in COA/DM Handler of Accounting Flow");
					isValid = false;
				}
				
				boolean isValidTranslationMapping = checkValidTransaltionMapping(coaDMEntryData.getTranslationMapping(), context, "COA/DM Handler");
		...
		for(AsyncCommunicationEntryData asynchCommunicationEntryData : communicationEntryDatas ){
			if (asynchCommunicationEntryData != null) {
				...
				if( Strings.isNullOrBlank(asynchCommunicationEntryData.getAcceptOnTimeout())){
					RestUtitlity.setValidationMessage(context,
									"Accept On Timeout must be specified  in "
											+ HandlerConstants.BROADCAST_HANDLER
											+ " of Accounting Flow, It could be 'true' or 'false'");
					isValid = false;
				}else if(BOOLEAN_REGEX.matcher(asynchCommunicationEntryData.getAcceptOnTimeout()).matches() == false){
					RestUtitlity.setValidationMessage(context,
							"Invalid value of Accept On Timeout [ " + asynchCommunicationEntryData.getAcceptOnTimeout()+" ] in "+ HandlerConstants.BROADCAST_HANDLER +" of Accounting Flow, It could be 'true' or 'false'");
					isValid = false;
				} 	
				...
				if( Strings.isNullOrBlank(asynchCommunicationEntryData.getWait())){
					RestUtitlity.setValidationMessage(context,
							"Wait for Response must be specified  in " + HandlerConstants.BROADCAST_HANDLER + " Handler of Accounting Flow, It could be 'true' or 'false' ");
					isValid = false;
				}else if(BOOLEAN_REGEX.matcher(asynchCommunicationEntryData.getWait()).matches() == false){
					RestUtitlity.setValidationMessage(context,
							"Invalid value of Wait for Response [ " + asynchCommunicationEntryData.getWait()+" ] in "+ HandlerConstants.BROADCAST_HANDLER +" of Accounting Flow, It could be 'true' or 'false'");
					isValid = false;
				} 	
	...
	private boolean checkValidESI(CommunicatorGroupData communicatorGroupData, ConstraintValidatorContext context, String flowType) {
		...
		if (communicatorGroupData != null) {
			...
			if( Collectionz.isNullOrEmpty(communicatorGroupData.getCommunicatorDataList()) == false ){
				...
				for (CommunicatorData communicatorData : communicatorGroupData.getCommunicatorDataList()) {
					...
					try {
						...
						if( Strings.isNullOrBlank(esiIId) ){
							RestUtitlity.setValidationMessage(context, "Server name must be specified in Server/Group of " + flowType+ " of Accounting Flow");
							isValid = false;
						}else{
							externalSystemBLmanager.getAcctExternalSystemInterfaceInstanceData(esiIId);
						}
					} catch (DataManagerException de) {
						RestUtitlity.setValidationMessage(context, "Configured ESI ["+esiIId+"] is invalid/Its type is invalid in " + flowType+ " of Accounting Flow");
						isValid = false;
					}
					...
					if( Strings.isNullOrBlank(loadFactor) ){
						RestUtitlity.setValidationMessage(context, "Load Factor name must be specified in Server/Group of " + flowType+ " of Accounting Flow");
						isValid = false;
					}else{
						...
						try {
							...
							if (loadFactorVal < 0 || loadFactorVal > 10) {
								RestUtitlity
										.setValidationMessage(
												context,
												"Invalid value of Load factor [ "+loadFactor+" ] in "+ flowType+" handler of Accounting Flow. It must be between 0 to 10");
								isValid = false;
							}
							...
							RestUtitlity
									.setValidationMessage(
											context,
											"Invalid value of Load factor [ "+loadFactor+" ]  in "+flowType+" handler of Accounting Flow. It must be Digit only[between 0 to 10]");
							isValid = false;
						}
					}
				...
				RestUtitlity.setValidationMessage(context, "Aleast one ESI mapping is required in "+ flowType +" handler of Accounting Flow");
				isValid = false;
			}
		} else {
			RestUtitlity.setValidationMessage(context, "Atleast one ESI Mapping is Required in "+ flowType +" Handler of Accounting Flow");
			isValid = false;
		}
						...
						try{
							...
							for(PrimaryDriverDetail primaryDriverDetail : primaryDriverDataList){
								...
								if( Strings.isNullOrBlank(primaryDriverName) ){
									 RestUtitlity.setValidationMessage(context, "Primary Driver name must be specified in CDR hanlder of Accounting Flow");
									 isValid = false;
								}else{
									boolean isDriverMatched = false;
									 ...
									 if( isDriverMatched == false ){
										 RestUtitlity.setValidationMessage(context, "Invalid type of Primary Driver name in CDR hanlder of Accounting Flow");
										 isValid = false;
									 }
								}
							}
							...
							if( Collectionz.isNullOrEmpty(cacheDriverDetails) == false ){
								...
								for(SecondaryAndCacheDriverDetail secondaryAndCacheDriverDetail : cacheDriverDetails){
									...
									if( Strings.isNullOrBlank(secondaryDriverName) == false ){
										 ...
										 if( isDriverMatched == false ){
											 RestUtitlity.setValidationMessage(context, "Invalid type of Secondary Driver name in CDR hanlder of Accounting Flow");
											 isValid = false;
										 }
									}
								}
