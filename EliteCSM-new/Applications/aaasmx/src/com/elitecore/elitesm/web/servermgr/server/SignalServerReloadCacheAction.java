...
public class SignalServerReloadCacheAction  extends BaseDictionaryAction{
    ...
    public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response) throws Exception{
    	...
    	try {
    		...
    		if(netServerId != null){
    			...
    			ServerReloadResponse serverReloadResponse = (ServerReloadResponse)remoteCommunicationManager.execute(MBeanConstants.CONFIGURATION,"reloadCache",null,null);
    			Logger.logInfo(MODULE,serverReloadResponse.getErrorMessage());
    			if(serverReloadResponse.isReload()){
    				request.setAttribute("responseUrl","/viewReloadCacheDetails.do?netServerId="+netServerId);
    				ActionMessage message = new ActionMessage("servermgr.server.reload.cache.success");
    				ActionMessages messages = new ActionMessages();
    				messages.add("information",message);
    				saveMessages(request,messages);
    				return mapping.findForward(SUCCESS_FORWARD);
    			}else{
    				throw new Exception(serverReloadResponse.getErrorMessage());
    			}
    		}
