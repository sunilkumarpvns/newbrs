...
public class AddRadiusPolicyCheckItemAction extends BaseDictionaryAction {
	...
	private static final String CREATE_CHECKITEM_FORWARD = "createRadiusPolicyCheckItem";
	...
	public ActionForward execute( ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) throws Exception{
		if(checkAccess(request, ACTION_ALIAS)){  
			...
			AddRadiusPolicyCheckItemForm radiusPolicyCheckItemForm = (AddRadiusPolicyCheckItemForm)form;
			AddRadiusPolicyForm radiusPolicyForm = (AddRadiusPolicyForm)request.getSession().getAttribute("addRadiusPolicyForm");
			ActionErrors errors = new ActionErrors();
			String action = radiusPolicyCheckItemForm.getCheckAction();
			...
			request.setAttribute("addRadiusPolicyCheckItemForm",radiusPolicyCheckItemForm);
			if(action != null){
				if(action.equalsIgnoreCase("next")){
					radiusPolicyForm.setCheckItem(radiusPolicyCheckItemForm.getCheckItem());
					radiusPolicyCheckItemForm.setParameterValue("");
					request.getSession().setAttribute("addRadiusPolicyForm", radiusPolicyForm);
					if(radiusPolicyForm.isRejectItemForward())
					...
					IDictionaryParameterDetailData dicParamDetailData = dictionaryBLManager.getOnlyDictionaryParametersByAttributeId(radiusPolicyCheckItemForm.getStrAttributeId());
					radiusPolicyCheckItemForm.setStatus(radiusPolicyForm.getStrStatus());
					radiusPolicyCheckItemForm.setParameterValue("");
					if(dicParamDetailData != null) {
						request.setAttribute("preDefinedValueMap",dicParamDetailData.getPreDefinedValueMap());
					}
					return mapping.findForward(CREATE_CHECKITEM_FORWARD);
				}
			}else{
				radiusPolicyCheckItemForm.setParameterValue("");
				return mapping.findForward(CREATE_CHECKITEM_FORWARD);     		
