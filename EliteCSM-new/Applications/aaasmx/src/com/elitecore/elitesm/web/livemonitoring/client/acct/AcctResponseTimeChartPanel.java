...
public class AcctResponseTimeChartPanel extends AbstractChartPanel{
	


	private static final String  title = "<b> Accounting Service Response Time</b>";
	private static final String MODULE = "AcctResponseTimeChartPanel";
	...
	public AcctResponseTimeChartPanel(){
		super();
		try{
			...
			Log.info("AcctResponseTimeChartPanel created.");
		}catch(Exception e){
			Log.error("Error in creating AcctResponseTimeChartPanel", e);
		}
	}
	...
	public void setData(List<Long[]>result){
		try{
			if(result!=null){
				...
				for(int i=0;i<size;i++){
					...
					if(data!=null){
						...
						Long aveResponseTime = data[GraphConstants.IDX_ACCT_RESPONSETIME_TOTAL_REPSONSE_TIME];
						Long avgQueueTime = data[GraphConstants.IDX_ACCT_RESPONSETIME_QUEUE_TIME];
						Long avgRMCommTime = data[GraphConstants.IDX_ACCT_RESPONSETIME_RMCOMMUNICATION_TIME];
			...
			Log.error("Error in setdata for AcctResponseTimeChartPanel", e);
		}
