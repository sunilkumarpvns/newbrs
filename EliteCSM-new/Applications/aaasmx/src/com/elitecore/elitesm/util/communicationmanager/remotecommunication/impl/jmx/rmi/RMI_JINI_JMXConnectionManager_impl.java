 ...
 * @author :  kaushikvira
 */
package com.elitecore.elitesm.util.communicationmanager.remotecommunication.impl.jmx.rmi;
    ...
    public void init( String hostIp ,String port, String encServerIdentifier, boolean serverIdVerifyFlag ) throws CommunicationException, UnidentifiedServerInstanceException, ConnectException {
        ...
        try {
            ...
            if(inetAddress instanceof Inet6Address) {
            	Logger.logInfo(MODULE, "Ip6 Address is used");
            	hostIp = new StringBuilder("[").append(hostIp).append("]").toString();
            }
            url = new JMXServiceURL("service:jmx:rmi:///jndi/rmi://" + hostIp + ":" + port + "/jmxrmi");
           // url = new JMXServiceURL("rmi", hostIp , Integer.parseInt(port),"/jndi/rmi://[" + hostIp + "]:" + port + "/jmxrmi");
            ...
            if(serverIdVerifyFlag){
                ...
                if(serverInstanceId != null){
                    if(!serverInstanceId.equals(encServerIdentifier)){
                        throw new UnidentifiedServerInstanceException("Server Identifier invalid, Expected: " + encServerIdentifier + 
                        		" arrived: " + serverInstanceId );
                    }
                }else{
                	throw new UnidentifiedServerInstanceException("Server Identifier invalid, Expected: " + encServerIdentifier + 
                    		" arrived: " + serverInstanceId);            		
