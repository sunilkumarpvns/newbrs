package com.elitecore.elitesm.ws.rest.serverconfig.server.configurations.resourcemanager.clients;
...
public class ClientsConfigurationController {
	...
	public Response getByNameFromQuery(@NotEmpty(message = RestValidationMessages.NAME_NOT_FOUND_QUERY_PARAMETER)
		...
		if(netServerBLManager.isRMManager(netServerInstanceData.getNetServerTypeId()) == false){
			return Response.ok(RestUtitlity.getResponse("You are not allow to retrive Clients configuration, Reason : Entered server name's type is EliteCSM Server", ResultCode.INVALID_INPUT_PARAMETER)).build();
		}
	...
	public Response updateClientsConfigurationByQueryParam(@Valid ClientsConfigurationData clientsConfigData,@NotEmpty(message = RestValidationMessages.NAME_NOT_FOUND_QUERY_PARAMETER) @QueryParam(value = "name") String serverInstanceName)
		...
		if(netServerBLManager.isRMManager(netServerInstanceData.getNetServerTypeId()) == false){
			return Response.ok(RestUtitlity.getResponse("You are not allow to update clients configuration, Reason : Entered server name's type is EliteCSM Server", ResultCode.INVALID_INPUT_PARAMETER)).build();
		}
	...
	public Response getClientsConfigurationHelp() throws FileNotFoundException, IllegalArgumentException,IOException {
		return RestUtitlity.getHelp(RestHelpConstant.ELITERMSERVER_CLIENTS_CONFIGURATION);
	}
}
