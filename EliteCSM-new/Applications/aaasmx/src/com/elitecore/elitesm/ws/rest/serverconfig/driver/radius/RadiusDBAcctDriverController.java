...
@Path("/")
public class RadiusDBAcctDriverController {
	
	private static final String RADIUS_DB_ACCT_DRIVER = "Radius DB Acct Driver";
	...
	public Response create(@Valid ListWrapper<DriverInstanceData> driverInstanceDataList, @Context UriInfo uri) throws DataManagerException {
		...
		for(DriverInstanceData driverInstanceData : list){
			...
			driverInstanceData.setDriverTypeId(DriverTypeConstants.RADIUS_DB_ACCT_DRIVER);
		...
		return Response.ok(RestUtitlity.getResponse(RADIUS_DB_ACCT_DRIVER, "(s) created successfully", responseMap,
	...
	public Response create(@Valid DriverInstanceData driverInstanceData) throws DataManagerException {
		...
		return  Response.ok(RestUtitlity.getResponse("Radius DB Acct Driver created successfully")).build();
	}
	...
	@QueryParam(value = "name") String name) throws DataManagerException {
		...
		try{
			...
			return  Response.ok(RestUtitlity.getResponse("Radius DB Acct Driver updated Successfully.Database connection is not found")).build();
		}
		
		return  Response.ok(RestUtitlity.getResponse("Radius DB Acct Driver updated successfully")).build();
	}
	...
	public Response	deleteByQueryParam(@NotEmpty(message = RestValidationMessages.NAME_NOT_FOUND_QUERY_PARAMETER) 
		...
		return  Response.ok(RestUtitlity.getResponse("Radius DB Acct Driver(s) deleted successfully")).build();
	}
	...
	public Response getHelp() throws FileNotFoundException, IllegalArgumentException, IOException{
		return RestUtitlity.getHelp(RestHelpConstant.RADIUS_DB_ACCT);
	}
