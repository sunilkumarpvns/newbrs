...
**/
@Path("/")
public class RadiusDBAuthDriverController {

	private static final String RADIUS_DB_AUTH_DRIVER = "Radius DB Auth Driver";
	...
	throws DataManagerException {
		...
		dbAuthDriverData.setCacheable(driverInstanceData.getCacheable());
	...
	public Response create(@Valid DriverInstanceData driverInstance) throws DataManagerException {
		...
		driverInstance.setDriverTypeId(DriverTypeConstants.RADIUS_DB_AUTH_DRIVER);
		...
		driverInstance.setCacheable(dbAuthDriverData.getCacheable());
		...
		if(errorMsg.equals(ConfigConstant.DATABASE_CONNECTION_FAILED)) {
			return Response.ok(RestUtitlity.getResponse("Radius DB Auth Driver created successfully.Database connection is not found.Subscriber Profile Management will not work properly")).build();
		} else if (errorMsg.equals(ConfigConstant.TABLE_DOES_NOT_EXIST)) {
			return Response.ok(RestUtitlity.getResponse("Radius DB Auth Driver created successfully.Subscriber Profiler Table [ "+dbAuthDriverData.getTableName()+" ] is not found. Subscriber Profile Management will not work properly")).build();
		} else {
			return Response.ok(RestUtitlity.getResponse("Radius DB Auth Driver created successfully")).build();	
		}
	}
	...
	public Response create(@Valid ListWrapper<DriverInstanceData> driverInstanceDataList, @Context UriInfo uri) throws DataManagerException {
		...
		if(!errorMessage.equals(ConfigConstant.SUCCESS)){
			return Response.ok(RestUtitlity.getResponse(RADIUS_DB_AUTH_DRIVER,
			...
			return Response.ok(RestUtitlity.getResponse(RADIUS_DB_AUTH_DRIVER, "(s) created successfully", responseMap,
	...
	@QueryParam(value = "name") String name) throws DataManagerException {
		...
		DBAuthDriverData dbAuthDriverData = driverBlManager.getDBDriverByDriverInstanceId(driverInstanceData.getDriverInstanceId());
		driverInstance.setCacheable(updateDBAuthDriverData.getCacheable());
		...
		try{
			...
			message ="Radius DB Auth Driver Updated Successfully.Database connection is not found.Subscriber Profile Management will not work properly";
		} catch (TableDoesNotExistException te) {
			message = "Radius DB Auth Driver Updated Successfully.Subscriber Profiler Table [ "+updateDBAuthDriverData.getTableName()+" ] is not found. Subscriber Profile Management will not work properly";
		}	
			driverInstance.setLastModifiedByStaffId(staffData.getStaffId());
			driverInstance.setLastModifiedDate(new Timestamp(System.currentTimeMillis()));
		...
		if(Strings.isNullOrBlank(message) == false) {
			...
			return Response.ok(RestUtitlity.getResponse("Radius DB Auth Driver updated successfully")).build();
	}
	...
	public Response	deleteByQueryParam(@NotEmpty(message = RestValidationMessages.NAME_NOT_FOUND_QUERY_PARAMETER) 
		...
		return Response.ok(RestUtitlity.getResponse("Radius DB Auth Driver(s) deleted successfully")).build();
	}
	...
	public Response getHelp() throws FileNotFoundException, IllegalArgumentException, IOException{
		return RestUtitlity.getHelp(RestHelpConstant.RADIUS_DB_AUTH);
	}
}
