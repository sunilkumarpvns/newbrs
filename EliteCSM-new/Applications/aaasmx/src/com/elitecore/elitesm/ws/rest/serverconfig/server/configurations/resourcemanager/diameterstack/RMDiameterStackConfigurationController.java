package com.elitecore.elitesm.ws.rest.serverconfig.server.configurations.resourcemanager.diameterstack;
...
import com.elitecore.elitesm.ws.rest.serverconfig.server.configurations.resourcemanager.diameterstack.data.DiameterStackConfigurationData;
import com.elitecore.elitesm.ws.rest.util.RestUtitlity;

public class RMDiameterStackConfigurationController {
	...
	public Response getByNameFromQuery(@NotEmpty(message = RestValidationMessages.NAME_NOT_FOUND_QUERY_PARAMETER)
		...
		if(netServerBLManager.isRMManager(netServerInstanceData.getNetServerTypeId()) == false){
			return Response.ok(RestUtitlity.getResponse("You are not allow to retrive Diameter stack configuration, Reason : Entered server name's type is EliteCSM Server", ResultCode.INVALID_INPUT_PARAMETER)).build();
		}
	...
	public Response updateDiameterStackConfigurationByQueryParam(@Valid DiameterStackConfigurationData diameterStackConfigData,@NotEmpty(message = RestValidationMessages.NAME_NOT_FOUND_QUERY_PARAMETER) @QueryParam(value = "name") String serverInstanceName)
		...
		if(netServerBLManager.isRMManager(netServerInstanceData.getNetServerTypeId()) == false){
			return Response.ok(RestUtitlity.getResponse("You are not allow to update diameter stack configuration, Reason : Entered server name's type is EliteCSM Server", ResultCode.INVALID_INPUT_PARAMETER)).build();
		}
	...
	public Response getDiameterStackConfigurationHelp() throws FileNotFoundException, IllegalArgumentException,IOException {
		return RestUtitlity.getHelp(RestHelpConstant.ELITERMSERVER_DIAMETER_STACK_CONFIGURATION);
	}
}
