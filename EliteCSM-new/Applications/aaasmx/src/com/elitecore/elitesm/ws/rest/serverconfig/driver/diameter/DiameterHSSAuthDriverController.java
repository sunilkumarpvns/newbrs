...
@Path("/")
public class DiameterHSSAuthDriverController {

	private static final String DIAMETER_HSS_AUTH_DRIVER = "Diameter HSS Auth Driver";
	...
	@GET
	@Path("/{name}")
	public Response getByPathParam(@PathParam(value = "name") String driverName)
			throws DataManagerException {
	...
	public Response create(@Valid DriverInstanceData driverInstance) throws DataManagerException {
		...
		driverInstance.setDriverTypeId(DriverTypeConstants.DIAMETER_HSS_AUTH_DRIVER);
		...
		return Response.ok(RestUtitlity.getResponse("Diameter HSS Auth Driver  created successfully")).build();
	}
	...
	public Response create(@Valid ListWrapper<DriverInstanceData> driverInstanceDataList, @Context UriInfo uri)throws DataManagerException {
		...
		return Response.ok(RestUtitlity.getResponse(DIAMETER_HSS_AUTH_DRIVER,"(s) created successfully", responseMap,
				URLInfo.isPartialSuccess(uri), URLInfo.isStatusRecord(uri))).build();
	}

	@PUT
	public Response updateByQueryParam(@Valid DriverInstanceData driverInstance,@NotEmpty(message = RestValidationMessages.NAME_NOT_FOUND_QUERY_PARAMETER) @QueryParam(value = "name") String name)
			throws DataManagerException {
		...
		return Response.ok(RestUtitlity.getResponse("Diameter HSS Auth Driver updated successfully")).build();
	}
	...
	@DELETE
	public Response deleteByQueryParam(@NotEmpty(message = RestValidationMessages.NAME_NOT_FOUND_QUERY_PARAMETER) @QueryParam(value = "name") String driverInstanceName)
			throws DataManagerException {
		...
		return Response.ok(RestUtitlity.getResponse("Diameter HSS Auth Driver(s) deleted successfully")).build();
	}
	...
	public Response getHelp() throws FileNotFoundException, IllegalArgumentException, IOException {
		return RestUtitlity.getHelp(RestHelpConstant.DIAMETER_HSS_AUTH);
	}
