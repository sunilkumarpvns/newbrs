...
@Path("/")
public class SysLogListenerController {

	private static final String SYS_LOG_LISTENER_CONFIGURATION = "SysLog Listener configuration";

	@GET
	public Response getByNameFromQuery(@NotEmpty(message = RestValidationMessages.NAME_NOT_FOUND_QUERY_PARAMETER)
	@QueryParam(value = "name") String sysLogListenerName) throws DataManagerException, JAXBException {

		AlertListenerBLManager alertListenerBLManager = new AlertListenerBLManager();
		AlertListenerData alertListenerData = getAlertListenerData(sysLogListenerName);
		List<AlertTypeData> allAlertData = alertListenerBLManager.getAlertTypeDataList();
		getAlertListenerRelationalData(alertListenerData, allAlertData);
	...
	public Response create(@Valid AlertListenerData alertListenerData) throws DataManagerException {
		...
		alertListenerData.setTypeId(AlertListenerConstant.SYS_ALERT_LISTENER_TYPE_ID);
		alertListenerBLManager.create(alertListenerData, staffData);

		return Response.ok(RestUtitlity.getResponse("SysLog Listener configuration created successfully")).build();
	}
	...
	public Response create(@Valid ListWrapper<AlertListenerData> trapListenerDataList,@Context UriInfo uri) throws DataManagerException {
		...
		return Response.ok(RestUtitlity.getResponse(SYS_LOG_LISTENER_CONFIGURATION, "(s) created successfully",
	...
	@QueryParam(value = "name") String alertListenerName)  throws DataManagerException, JAXBException {
		...
		return Response.ok(RestUtitlity.getResponse("SysLog configuration updated successfully")).build();
	...
	@QueryParam(value = "name") String alertListenerName) throws DataManagerException {
		...
		return Response.ok(RestUtitlity.getResponse("SysLog configuration(s) deleted successfully")).build();
	}
	...
	private void getAlertListenerRelationalData(AlertListenerData alertListenerData, List<AlertTypeData> allAlertData) {
		alertListenerData.setAlertSysLogAlertListenerData((SYSLogAlertListenerData) alertListenerData.getAlertListener());
	...
	private void setAlertListenerData(AlertListenerData alertListenerData) throws DataManagerException {
		SYSLogAlertListenerData syslogListenerData = alertListenerData.getAlertSysLogAlertListenerData();
		try {
			if (alertListenerData.getAlertTrapListenerData() != null || alertListenerData.getAlertFileListenerData() !=null) {
				throw new DataValidationException();
			}
			if (syslogListenerData == null) {
				throw new InputParameterMissingException();
			} else {
				alertListenerData.setAlertListener(syslogListenerData);
			}

		} catch (DataValidationException dve) {
			throw new DataManagerException("Invalid Parameter given for Syslog Listener Configuration.", dve);
		} catch (InputParameterMissingException ipm) {
			throw new DataManagerException("Insufficient basic paremerts for Syslog Listener Configuration.", ipm);
		}
	}
	...
	public Response getHelp() throws FileNotFoundException, IllegalArgumentException, IOException {
		return RestUtitlity.getHelp(RestHelpConstant.SYSLOG_LISTENER);
	}
	...
	private AlertListenerData getAlertListenerData(String fileListenerName) throws DataManagerException {
		...
		try {
			...
			if (AlertListenerConstant.SYS_ALERT_LISTENER_TYPE_ID.equals(alertListenerData.getTypeId()) == false) {
