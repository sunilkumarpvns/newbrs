var com_github_culmat_jsTreeTable =  (function(){
	...
	function renderTree(tree, childrenAttr, idAttr, attrs, renderer, tableAttributes) {
		...
		function render(node, parent) {
			...
			if (parent) {
				if(oldLevel > node['data-tt-level']){
					var tempOldLevel = oldLevel;
					while(tempOldLevel > node['data-tt-level']){
						var indexOfLastParent = parentId.trim().lastIndexOf(" ");
						if(indexOfLastParent > 0){
							parentId=parentId.substring(0,indexOfLastParent);
						}
						tempOldLevel--;
					}
				}
				
				var removedSpcId = parent[idAttr].replace(/ /g,'');
				if(parentId.indexOf(removedSpcId)  == -1){
					parentId += ' ' + removedSpcId;
				}
				
				oldLevel = node['data-tt-level'];
				$(tr).attr('data-tt-parent-id', parentId);
				$(tr).attr('data-tt-id', parentId + ' ' +node[idAttr].replace(/ /g,''))
			}else{
				parentId = node[idAttr].replace(/ /g,'');
				$(tr).attr('data-tt-id', parentId)
			}
			if (renderer) {
				renderer($(tr), node)
			}else if (attrs) {
	...
	function appendTreetable(tree, options) {
		if ( tree.length == 0){
			var tr = $('<tr>')
			tr.append($("<td style='padding-right:140px' align='center' class='labeltext no-history-details ' >").append("No changes found in this update "))
			
			options.mountPoint.append($('<table border="0" width="100%"/>').append(tr)) ;
			return ;
		}
