...
//		  doc.write("<div id=domRoot></div>") //transition between regular flow HTML, and node-insert DOM DHTML
		  insertAtObj = getElById("mydiv")
			 // alert(docW);
		  insertAtObj.insertAdjacentHTML("beforeEnd", "<table><tr><td>" + docW + "</td></tr></table>" )
	  }
	  else
		  doc.write(docW)
  }
...
{ 
  ...
  { 
    ...
    if (browserVersion > 0) 
      docW = docW + "onClick='" + this.hreference + "javascript:clickOnFolder(\""+this.getID()+"\");return false;'"
...
function initializeTree(pobj)
{
		
	USETEXTLINKS = pobj.usetextlinks 
	STARTALLOPEN = pobj.startallopen
	USEFRAMES = pobj.useframes
	USEICONS = pobj.useicons
	WRAPTEXT = pobj.wraptext
	PERSERVESTATE = pobj.perservestate
	ICONPATH = pobj.iconpath
	HIGHLIGHT = pobj.highlight
	HIGHLIGHT_COLOR =pobj.highlight_color
	HIGHLIGHT_BG    = pobj.highlight_bg
    BUILDALL = pobj.buildall
}
...
function initializeDocument(mydoc) 
{ 
	//alert("initializeDocument :: " +mydoc);
	if( mydoc)
		doc=mydoc;
