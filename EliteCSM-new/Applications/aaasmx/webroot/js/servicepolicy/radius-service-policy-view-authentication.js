...
var handlerPagePath = {"ProfileLookupDriver":"ViewProfileLookupDriver.jsp",
						"AuthenticationHandler":"ViewAuthenticationHandler.jsp",
						"AuthorizationHandler":"ViewAuthorizationHandler.jsp",
						"CDRGeneration":"ViewCDRGeneration.jsp",
						"ConcurrencyHandler":"ViewConcurrencyHandler.jsp",
						"ProxyCommunication":"ViewProxyCommunication.jsp",
						"BroadcastingCommunication":"ViewBroadcastCommunication.jsp",
						"Plugin":"ViewPlugin.jsp",
						"coaDMGen":"ViewCOADMGeneration.jsp",
    					"RadiusIMDGConcurrency":"ViewConcurrencyIMDGHandler.jsp",
...
function validateConcurrencyHandler(handlerName){
	var isConcurrencyTestPassed =  true;
	if(handlerName == 'ConcurrencyHandler' ){
    	 var sessionManagerIds = $('#sessionManagerId').val();
    	 if(sessionManagerIds == 0){
    		 alert('You are not allowed to add Concurrency Handler. First, configure session manager in Basic Details.');
    		 isConcurrencyTestPassed = false;
    	 }
    }
	 return isConcurrencyTestPassed;
}
...
function validateAuthenticationHandler(handlerName,handlerTable){
	...
	if(handlerName == 'AuthenticationHandler'){
		if($('#'+handlerTable).find('.form_auth').length == 1){
			alert('You will not allowed to add Authentication Handler twice');
...
function loadDefaultServiceHandler(){
	for(var i=0;i<authdefaultHandler.length;i++){
		addNewHandler(authdefaultHandler[i],'authenticationTbl',true,true);
	}
	for(var i=0;i<authAdditional.length;i++){
		addNewHandler(authAdditional[i],'additionalTbl',true,true);
	}	
}
