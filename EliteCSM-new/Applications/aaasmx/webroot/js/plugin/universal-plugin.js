...
function validateUniversalPreAuthPluginData(){
...
function validateUniversalPostAuthPluginData(){
	var returnValue = true;
...
function fetchPreAndPostPluginData(){
	...
	$('#plugin-mapping-table').find('.universal_pre_plugin_div').each(function(){
		...
		var rejectOnCheckedItemNotFound = $(this).find("select[name='rejectOnCheckedItemNotFound']").val();
		...
		prePolicyLists.push({
				'name' 							: name,
				'action' 						: action,
				'rejectOnCheckedItemNotFound'	: rejectOnCheckedItemNotFound,
	...
	universalPluginPolicyJson.push({
		...
		'prePolicyLists' 	: prePolicyLists,
		'postPolicyLists' 	: postPolicyLists
	});
...
function validatePlugin(){
	...
	/* Fetching Pre and Post Mappings Data*/
	fetchPreAndPostPluginData();

	if( !validateUniversalPreAuthPluginData() ){
		return false;
	}else if( !validateUniversalPostAuthPluginData() ){
		return false;
	...
	$.each(handlerData, function( key, value ){
		...
		if(key === 'parameterDetailsForPlugin'){
			...
			if( key == 'name'){
				...
				var element = $("#" + mappingTable +" > tbody > tr:last-child").find("select[name='rejectOnCheckedItemNotFound']");
...
function validatePluginForm(){
	...
	if($('#pluginName').val() == ""){
		...
		$('#pluginName').focus();
		return;
	}else if( !validateUniversalPreAuthPluginData() ){
		return false;
	}else if( !validateUniversalPostAuthPluginData() ){
		return false;
	}else if( !isUnique("policy-name") ){
		...
		document.forms['uniPlugin'].submit();
	}
}