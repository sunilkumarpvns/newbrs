...
function validateGroovyPlugin(){
	...
	if($('#pluginName').val() == ""){
		...
		document.forms['diameterGroovyPlugin'].pluginName.focus();
		return;
	}else{
...
function viewGroovyFileNew(obj){
	var groovyFileName = $(obj).parent().parent().find('.groovy-label-text').html();
		...
		if(groovyFileName != null && groovyFileName.length > 0){
			...
			$('#groovyFileName').val(groovyFileName);
			$('#redirectUrlForViewFile').val('updateDiameterGroovyPlugin.do');
			
			document.forms['diameterGroovyPlugin'].submit();
		}
	}
//This function has defined for set the index to groovyFile because Bean.Poputil throws error due to no index apply on attribute. 
