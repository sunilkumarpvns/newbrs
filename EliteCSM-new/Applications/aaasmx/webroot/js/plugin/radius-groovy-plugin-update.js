	 ...
	 $('#groovyFilePlugin').filer({
         ...
         templates: {
             ...
             itemAppend: '<tr class="jFiler-item">\
                 			<td class="labeltext tblfirstcol groovyName"> <span class="groovy-label-text" onClick="viewGroovyFileNew(this);">{{fi-name}}</span></td>\
            	 			<td  class="labeltext tblrows" align="center"> {{fi-lastModifiedDate}} </td>\
                			<td class="labeltext tblrows" align="center"> {{fi-size2}} </td>\
...
var groovyFileName = $(obj).parent().parent().find('.groovy-label-text').html();
	...
	if(groovyFileName != null && groovyFileName.length > 0){
		...
		$('#groovyFileName').val(groovyFileName);
		$('#redirectUrlForViewFile').val('updateRadiusGroovyPlugin.do');
