...
function getReadyFiler(module){
	 $('#groovyFilePlugin').filer({
         ...
         theme: "dragdropbox",
         templates: {
             box: '<table class="jFiler-item-list box groovy-table-css" cellspacing="0" cellpadding="0" border="0" width="80%"><tr><th align="left" class="tbl-header-bold" width="60%">Groovy File Name</th><th align="center" class="tbl-header-bold" width="20%">Last Modified Time</th><th align="center" class="tbl-header-bold" width="10%">Size</th><th align="center" class="tbl-header-bold" width="10%">Edit</th><th align="center" class="tbl-header-bold" width="10%">Remove</th></tr></table>',
             item: '<tr class="jFiler-item">\
                        ...
                        <td  class="labeltext tblrows" align="center"> {{fi-size2}}</td>\
            	 		<td  class="labeltext tblrows" align="center"><span class="edit-groovy-file-new" title="Not Available Yet"></span>&nbsp;</td>\
             ...
             itemAppend: '<tr class="jFiler-item">\
                 			<td class="labeltext tblfirstcol groovyName"> <span class="groovy-label-text" onClick="viewScriptFileNew(this);">{{fi-name}}</span></td>\
            	 			<td  class="labeltext tblrows" align="center"> {{fi-lastModifiedDate}} </td>\
                			<td class="labeltext tblrows" align="center"> {{fi-size2}} </td>\
            	 			<td  class="labeltext tblrows" align="center"><span class="edit-groovy-file" onclick="editGroovyFile(this);"></span>&nbsp;</td>\
                			<td  class="labeltext tblrows" align="center"><span class="delete remove-proxy-server jFiler-item-trash-action" onclick="deleteMe(this);"></span>&nbsp;</td>\
                		 </tr>',
...
function validateScript(){
	if($('#pluginName').val() == ""){
		...
		document.forms['radiusGroovyPlugin'].pluginName.focus();
		return;
	}else{
...
function validateGroovyScript(){
	if($('#scriptName').val().trim() == ""){
		alert('Script Instance Name must be specified');
		$('#scriptName').focus();
	}else if(!isValidName) {
		alert('Enter Valid Script Instance Name');
		document.forms['groovyScript'].scriptName.focus();
		return;
	} else if ($('#selectedScript').val() == "") {
		alert('Select atleast one Script Type');
		 ...
		 if(value.fileName == fileName){
			  ...
			  var f = new File([fileData], fileName);
		        ...
		        reader1.onload = function(e1) {
		        	 ...
		        	 editor.getSession().setValue(e1.target.result);
...
function deleteMe(spanObject){
	...
	findAndRemove('fileName', groovyFileName);
...
function findAndRemove(property, value) {
	   $.each(fileArray, function(index, result) {
...
function findAndReplace(property, value,fileContent,lastModified){
	var isContentReplace = false; 
	$.each(fileArray, function(index, result) {
	      if(result[property] == value) {
	          //Remove from array
	    	  fileArray[index].fileContent =fileContent;
	    	  fileArray[index].lastModified=lastModified;
...
function onchnage(){
	...
	for (var i = 0; i < files.length; i++){
         ...
         groovyFileReader.onload = (function(theFile){
	            ...
	            return function(e){
	            	var isFileExist = findAndReplace('fileName',fileName,e.target.result,lastModified);
	            	if( !isFileExist ){
...
//This function has defined for set the index to groovyFilePlugin because Bean.Poputil throws error due to no index apply on attribute.
function setFileInputIndex(cssClass){
	$("form").find("." +cssClass ).each(function(e,v){
