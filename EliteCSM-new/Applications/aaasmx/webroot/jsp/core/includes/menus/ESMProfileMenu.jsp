...
<%
	...
	SystemLoginForm userSession = (SystemLoginForm)request.getSession().getAttribute("radiusLoginForm");    
%>
...
oCMenu.level[0].offsetY=-55 //The Y offset of the submenus of this item. This does not affect the first submenus, but you need it here so it can be the default value for all levels. - Value: px
...
*/
<%if(!(userSession.getSystemUserName().equalsIgnoreCase("ProfileAdmin"))) {%>
//RADIUS POLICY



<%if(true){%>
		oCMenu.makeMenu('top2','','&nbsp;Dictionary')
<%if(true){%>
			oCMenu.makeMenu('sub2-1-1','top2','&nbsp;Dictionary Management','<%=basePath1%>/listDictionary.do?isMenuCall=yes')
<%}
	}%>

<%if(true) {%>
	oCMenu.makeMenu('top3','','&nbsp;Radius')
<%if(true) {%>	
	oCMenu.makeMenu('sub3-1','top3','&nbsp;Policies')
<%if(true) {%>	
	oCMenu.makeMenu('sub3-1-1','sub3-1','&nbsp;Search Radius Policy','<%=basePath1%>/initSearchRadiusPolicy.do?isMenuCall=yes')
<%if(true) {%>
	oCMenu.makeMenu('sub3-1-2','sub3-1','&nbsp;Create Radius Policy','<%=basePath1%>/addRadiusPolicy.do?isMenuCall=yes&action=new')
<%if(true) {%>
	oCMenu.makeMenu('sub3-1-3','sub3-1','&nbsp;Search Access Policy','<%=basePath1%>/initSearchAccessPolicy.do?isMenuCall=yes')
<%if(true) {%>
	oCMenu.makeMenu('sub3-1-4','sub3-1','&nbsp;Create Access Policy','<%=basePath1%>/initCreateAccessPolicy.do?isMenuCall=yes')
<%}
						}
					}
				}
...
<%if(true){%>
        oCMenu.makeMenu('top4','','&nbsp;Resource Manager')

<%if(true){%>
            oCMenu.makeMenu('sub4-1','top4','&nbsp;IP Pool')
<%if(true){%>
                oCMenu.makeMenu('sub4-1-1','sub4-1','&nbsp;Search IP Pool','<%=basePath1%>/initSearchIPPool.do?isMenuCall=yes')
<%}%>

<%if(true){%>                                                                                                                                                                                                                                                                                                                                       
                oCMenu.makeMenu('sub4-1-2','sub4-1','&nbsp;Create IP Pool','<%=basePath1%>/initCreateIPPool.do?isMenuCall=yes')                                                                                                                                                                                                                
<%}                                                                                                                                                                                                                                                                                                                                
        }%>

<%if(true){%>
            oCMenu.makeMenu('sub4-2','top4','&nbsp;Concurrent Login Policy')

<%if(true){%>
                oCMenu.makeMenu('sub3-2-1','sub4-2','&nbsp;Search Conc. Login Policy','<%=basePath1%>/initSearchConcurrentLoginPolicy.do?isMenuCall=yes')
<%}
            
            if(true){%>
                oCMenu.makeMenu('sub3-2-2','sub4-2','&nbsp;Create Conc. Login Policy','<%=basePath1%>/addConcurrentLoginPolicy.do?isMenuCall=yes&action=new')
<%}
        }%>
...
<%if(true){%>
        oCMenu.makeMenu('top6','','&nbsp;Mediation')
<%if(true){%>
            oCMenu.makeMenu('sub6-1','top6','&nbsp;Processing Rules','<%=basePath1%>/listGroupProcessingRuleType.do?isMenuCall=yes')
<%}
    }%>  

<%if(true){%>
		oCMenu.makeMenu('top7','','&nbsp;Servers')
<%if(true){%>
			oCMenu.makeMenu('sub7-1','top7','&nbsp;List Servers','<%=basePath1%>/listNetServerInstance.do?isMenuCall=yes')
<%}if(true){%>
			oCMenu.makeMenu('sub7-2','top7','&nbsp;Create Server Instance','<%=basePath1%>/initCreateServer.do?isMenuCall=yes')
<%}%>
<%}%>
...
<%if(true){%>
           oCMenu.makeMenu('top5','','&nbsp;Staff')
<%if(true){%>
               oCMenu.makeMenu('sub5-1-1','top5','&nbsp;Create Staff','<%=basePath1%>/initCreateStaff.do?isMenuCall=ye')
<%}
        if(true) {%>
               oCMenu.makeMenu('sub5-1-2','top5','&nbsp;Search Staff','<%=basePath1%>/initSearchStaff.do?isMenuCall=yes')
<%}

    }%>

<%}// End of Block not related to Profile Admin
	if(userSession.getSystemUserName().equals("ProfileAdmin")) {	
//	if(true) {%>
		 oCMenu.makeMenu('top10','','&nbsp;Configuration Profile')
<%
	if(true) {
%>		 
		oCMenu.makeMenu('sub10-1-1','top10','&nbsp;Business Model Management','<%=basePath1%>/listBusinessModel.do?isMenuCall=yes')
<%
	if(true) {
%>
		oCMenu.makeMenu('sub10-1-2','top10','&nbsp;Business Module ','<%=basePath1%>/initBusinessModuleManagement.do?isMenuCall=yes')
<%
	if(true) {
%>		
		oCMenu.makeMenu('sub10-1-3','top10','&nbsp;Sub-Business Module','<%=basePath1%>/initSubBusinessModuleManagement.do?isMenuCall=yes')
<%
	  	  }
	    }
	  }
...
<%
	if(true){
%>
			oCMenu.makeMenu('top8','','&nbsp;Change Password','<%=basePath1%>/initChangePassword.do?isMenuCall=yes')
<%		
	}
%>
...
<%
	if(true){
%>
			oCMenu.makeMenu('top9','','&nbsp;Logout','<%=basePath1%>/logout.do?isMenuCall=yes')
<%		
	}
%>
