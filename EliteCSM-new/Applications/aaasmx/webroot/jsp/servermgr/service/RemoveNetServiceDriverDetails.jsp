...
<%@ page
	import="com.elitecore.elitesm.web.servermgr.service.forms.RemoveNetServiceDriverInstanceForm"%>
...
<html:form action="/removeNetServiceDriverInstance">
	<html:hidden name="removeNetServiceDriverInstanceForm" styleId="action"
		property="action" />
	<html:hidden name="removeNetServiceDriverInstanceForm"
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						<td class="tblheader-bold" colspan="3"><bean:message
								...
								key="servermgr.removedriverinstances" /></td>
					</tr>
					<tr>
						<td colspan="3">
							<table width="100%" cols="8" id="listTable" type="tbl-list"
								...
								<tr>
									...
									<td align="left" class="tblheader" valign="top" width="20%"><bean:message
											bundle="servermgrResources" key="servermgr.drivertype" /></td>
								</tr>
								<%
    if(lstDriverData != null && lstDriverData.size() > 0){
%>
								<logic:iterate id="netDriverInstanceData"
									name="removeNetServiceDriverInstanceForm"
									property="listDrivers"
									type="com.elitecore.elitesm.datamanager.servermgr.data.INetDriverInstanceData">
									<tr>
										<td align="center" class="tblfirstcol" valign="top"><input
											type="checkbox" name="select"
											value="<bean:write name="netDriverInstanceData" property="netDriverId"/>" />
										</td>
										<td align="right" class="tblfirstcol" valign="top"><%=(iIndex+1) %></td>
										<td align="left" class="tblrows" valign="top"><bean:write
												name="netDriverInstanceData" property="name" /></td>
										...
										<td align="left" class="tblrows"><bean:write
												name="netDriverInstanceData" property="description" /></td>
										<td align="left" class="tblrows"><logic:iterate
												id="netDriverTypeData" name="lstNetDriverType"
												type="com.elitecore.elitesm.datamanager.servermgr.data.INetDriverTypeData">
												<logic:equal name="netDriverTypeData"
													property="netDriverTypeId"
													value="<%=netDriverInstanceData.getNetDriverTypeId()%>">
...
%>
		...
		<tr>
			<td class="btns-td" valign="middle"><html:button
					property="c_btnDelete" onclick="removeServerInstance()" value="Delete"
				...
				onclick="javascript:location.href='<%=localBasePath%>/viewNetServiceInstance.do?netserviceid=<bean:write name="netServiceInstanceBean" property="netServiceId"/>'"
