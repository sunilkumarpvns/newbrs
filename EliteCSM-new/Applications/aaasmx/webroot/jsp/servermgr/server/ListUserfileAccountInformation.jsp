...
<html:form action="/miscUserfileDatasource">
	<html:hidden name="miscUserfileDatasourceForm" styleId="action"
		property="action" />
	<html:hidden name="miscUserfileDatasourceForm" styleId="pageNo"
		property="pageNo" value="<%=Integer.toString(currentPage)%>" />
	<html:hidden name="miscUserfileDatasourceForm"
		styleId="totalNumberOfRecord" property="totalNumberOfRecord"
		value="<%=Integer.toString(totalNumberOfRecord)%>" />
	<html:hidden name="miscUserfileDatasourceForm" styleId="totalPage"
		property="totalPage" value="<%=Integer.toString(totalPage)%>" />
	<html:hidden name="miscUserfileDatasourceForm"
		styleId="selectedFileName" property="selectedFileName"
		value="<%= selectedFileName%>" />
	<html:hidden name="miscUserfileDatasourceForm" styleId="netserverid"
		property="netserverid" value="<%= netserverid%>" />

	<html:hidden name="miscUserfileDatasourceForm" styleId="totalRecords"
		property="totalRecords"
		value="<%=Integer.toString(totalNumberOfRecord)%>" />
	<html:hidden name="miscUserfileDatasourceForm" styleId="totalPages"
		property="totalPages" value="<%=Integer.toString(totalPage)%>" />
	<html:hidden name="miscUserfileDatasourceForm" styleId="pageNumber"
		property="pageNumber" value="<%=Integer.toString(currentPage)%>" />

	<table width="100%" border="0" cellspacing="0" cellpadding="0"
		height="15%" align="right">
		<tr>
			<td class="table-header" width="100%" align="left"><bean:message
					bundle="servermgrResources"
					key="servermgr.userfile.userfiledatasourcelist" /></td>
		...
		<tr>
			<td valign="left" colspan="4"><input name="c_btnAdd"
				type="button" class="light-btn" onclick="addNewUserAccount()"
				id="c_btnAdd" value="   Add  "> &nbsp; <input type="button"
				name="c_btnDelete" value=" Delete   " class="light-btn"
				onclick="deleteDatasourceData();" /></td>
			<td class="btns-td" align="right" colspan="4">
				...
				%> <%
					if (previousPage != 0) {
				%> <a
				href="<%=localBasePath%>/listUserfileAccountInformation.do?action=paging&pageNo=1&netserverid=<%=netserverid%>&totalNumberOfRecord=<%=totalNumberOfRecord%>&totalPage=<%=totalPage%>&selectedFileName=<%=selectedFileName%>"><img
				...
				href="<%=localBasePath%>/listUserfileAccountInformation.do?action=paging&pageNo=<%=previousPage%>&netserverid=<%=netserverid%>&totalNumberOfRecord=<%=totalNumberOfRecord%>&totalPage=<%=totalPage%>&selectedFileName=<%=selectedFileName%>"><img
				...
				%> <a
				href="<%=localBasePath%>/listUserfileAccountInformation.do?action=paging&pageNo=<%=nextPage%>&netserverid=<%=netserverid%>&totalNumberOfRecord=<%=totalNumberOfRecord%>&totalPage=<%=totalPage%>&selectedFileName=<%=selectedFileName%>"><img
				...
				href="<%=localBasePath%>/listUserfileAccountInformation.do?action=paging&pageNo=<%=totalPage%>&netserverid=<%=netserverid%>&totalNumberOfRecord=<%=totalNumberOfRecord%>&totalPage=<%=totalPage%>&selectedFileName=<%=selectedFileName%>"><img
							...
							%>
										...
										<tr>
											<td align="center" class="tblleftlinerows" valign="top"
												...
												value="<%=keyArray[j]%>" /></td>
											<td align="center" class="tblrows"><%=(start + j)%></td>
											<td align="left" class="tblrows"><%="Account" + keyArray[j]%>
											</td>
											<%
									 ...
									 %>
											<td align="left" class="tblrows"><%=attributeColumn%></td>
											<td align="left" class="tblrows">
												<%
									 if(calledStationId!= null && !calledStationId.trim().equals("")) {
									%> <%=calledStationId%> <%}else { %> <%="-"%> <%} %>
											</td>
											<td align="left" class="tblrows">
												<%
									...
									%> <%=serviceType%> <%}else { %> <%="-"%> <%} %>
											...
											<td align="center" class="tblrows"><a
												href="<%=localBasePath%>/viewUserfileDatasourceUserAccount.do?netserverid=<%=netserverid%>&id=<%=keyArray[j]%>&filename=<bean:write name="listUserfileAccountInformationForm" property="selectedFileName"/> "><img
													src="<%=localBasePath%>/images/edit.jpg" alt="Edit"
													border="0"></a></td>
										</tr>
										...
										<tr>
											<td align="center" class="tblleftlinerows" valign="top"
												width="100%" colspan="11"><bean:message
