...
<%
	...
	DiameterPeerGroupForm diameterPeerGroupForm = (DiameterPeerGroupForm)request.getAttribute("diameterPeerGroupForm");
   	...
   	List diameterPeerGroupList = diameterPeerGroupForm.getDiameterPeerGroupList();
   	
   	String action = diameterPeerGroupForm.getAction();
   	Integer pageSize = Integer.parseInt(ConfigManager.get("TOTAL_ROW"));
   	long pageNo = diameterPeerGroupForm.getPageNumber();
   	long totalPages = diameterPeerGroupForm.getTotalPages();
   	long totalRecord = diameterPeerGroupForm.getTotalRecords();
   	String strTotalPages=String.valueOf(totalPages);
   	String strTotalRecords=String.valueOf(totalRecord);
   	String strName = diameterPeerGroupForm.getPeerGroupName();
   	String paramString = "peerGroupName=" + strName;
    int count=1;
%>
...
function removeData(){
    ...
    if(selectVar == false){
        alert('At least select Diameter Peer Group for remove process');
    }else{
        var msg;
        msg = '<bean:message bundle="alertMessageResources" key="alert.searchdiameterpeergroup.delete.query"/>';        
        var agree = confirm(msg);
        if(agree){            
       	    document.forms[0].action.value = 'deletePeerGroup';
        	document.forms[0].submit();
        }
    }
}
...
function navigatePageWithName(action,appendAttrbId) {
	...
	$("#newFormData").append("<input type='hidden' name='peerGroupName' value='"+val+"'>").submit();
}
...
<html:form action="/searchDiameterPeerGroup">
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td class="table-header" colspan="5">
										<bean:message bundle="diameterResources" key="diameterpeergroup.search" />
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td align="left" class="labeltext" valign="top" width="66%">
										<html:text styleId="peerGroupName" property="peerGroupName" size="30" maxlength="60" style="width:250px" tabindex="1" />
									</td>
								</tr>
								<tr>
									<td colspan="3">&nbsp;</td>
								...
								<tr>
									...
									<td class="labeltext" valign="middle" colspan="2">
										...
										<input type="button" name="Create" value="   Create   " class="light-btn" onclick="javascript:navigatePageWithName('createDiameterPeerGroup.do','peerGroupName');" tabindex="4">
									</td>
								</tr>
								<tr>
									<td colspan="3">&nbsp;</td>
										...
										<table width="99%" cellpadding="0" cellspacing="0" border="0">
											<tr>
												<td class="table-header" width="50%">
													<bean:message bundle="diameterResources" key="diameterpeergroup.list" />
												</td>
											...
											<tr>
												...
												<%
									  			...
									  			%> <%
										  			if (pageNo == 1) { 	%>
										  			<a href="searchDiameterPeerGroup.do?<%=paramString%>&action=list&pageNo=<%=pageNo + 1%>">
													...
													<a href="searchDiameterPeerGroup.do?<%=paramString%>&action=list&pageNo=<%=totalPages + 1%>">
													...
													%> <%
														if (pageNo - 1 == 1) {
													%> 
													<a href="searchDiameterPeerGroup.do?<%=paramString%>&action=list&pageNo=<%=1%>">
													...
													<a href="searchDiameterPeerGroup.do?<%=paramString%>&action=list&pageNo=<%=totalPages + 1%>"><img
													...
													<a 	href="searchDiameterPeerGroup.do?<%=paramString%>&action=list&pageNo=<%=pageNo - 1%>">
														...
														onmouseover="MM_swapImage('Image5','','<%=basePath%>/images/previous-hover.jpg',1)"
														onmouseout="MM_swapImgRestore()" alt="Previous" border="0"></a>
													<a href="searchDiameterPeerGroup.do?<%=paramString%>&action=list&pageNo=<%=pageNo + 1%>"><img
														src="<%=basePath%>/images/next.jpg" name="Image61"
														onmouseover="MM_swapImage('Image61','','<%=basePath%>/images/next-hover.jpg',1)"
														onmouseout="MM_swapImgRestore()" alt="Next" border="0">
													</a>
												...
												<td class="btns-td" valign="middle" colspan="2">
													<table width="100%" border="0" cellpadding="0" 	cellspacing="0" id="listTable">
														<tr>
															...
															<td align="left" class="tblheader" valign="top" width="30%">Name</td>
															<td align="left" class="tblheader" valign="top" width="60%">Description</td>
															<td align="center" class="tblheader" valign="top" width="40px">Edit</td>
														</tr>

														<%
															if (diameterPeerGroupList != null && !diameterPeerGroupList.isEmpty()) {
														%>

														 <logic:iterate id="obj" name="diameterPeerGroupForm" property="diameterPeerGroupList" type="DiameterPeerGroup">
															<tr>
																<td align="center" class="tblfirstcol">
																	<input type="checkbox" name="select" value="<bean:write name="obj" property="peerGroupId"/>" />
																</td>
																<td align="center" class="tblfirstcol">
																	<%=((pageNo-1)*pageSize)+count%>
																</td>
																<td align="left" class="tblrows">
																	<a href="viewDiameterPeerGroup.do?action=view&peerGroupId=<bean:write name="obj" property="peerGroupId"/>">
																		<bean:write name="obj" property="peerGroupName" />
																	</a>
																</td>
																<td align="left" class="tblrows"><%=EliteUtility.formatDescription(obj.getDescription())%>&nbsp;&nbsp;</td>
																<td align="center" class="tblrows">
																	<a href="updateDiameterPeerGroup.do?peerGroupId=<bean:write name="obj" property="peerGroupId"/>">
											...
											<tr>
												...
												<td class="btns-td" align="right">
													...
													%> <%
														if (pageNo - 1 == 1) {
													%> 
													<a href="searchDiameterPeerGroup.do?<%=paramString%>&action=list&pageNo=<%=1%>"><img
