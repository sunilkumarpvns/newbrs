...
<%
    ...
    List lstMSISDNBasedRoutingTable = msisdnBasedRoutingTableForm.getListMSISDNBasedRoutingTable();
    
    long pageNo = msisdnBasedRoutingTableForm.getPageNumber();
	long totalPages = msisdnBasedRoutingTableForm.getTotalPages();
	long totalRecord = msisdnBasedRoutingTableForm.getTotalRecords();
	...
	if(msisdnBasedRoutingTableForm.getAction()!=null){
		 strAction=msisdnBasedRoutingTableForm.getAction();
	}
...
function navigatePageWithType(action,appendAttrbId) {
	...
	var name = $("#"+appendAttrbId).attr("msisdnBasedRoutingTableName");
	var val = $("#"+appendAttrbId).val();
	$("#newFormData").append("<input id='hiddenName' type='hidden' name='"+name+"' value='"+val+"'>").submit();
}

function validateCreate(){
	var name= $('#msisdnBasedRoutingTableName').val();
	if(!isValidName){
		alert('Enter Valid Routing Table Name');
		$('#msisdnBasedRoutingTableName').focus()
		return false;
	}else if(name){
		 location.href='<%=basePath%>/createMSISDNBasedRoutingTable.do?name=' + name;
	}else{
				...
				<tr>
					<td width="100%" class="box">
						<table cellpadding="0" cellspacing="0" border="0" width="100%">
							<tr>
								<td colspan="3">
									<html:form action="/searchMSISDNBasedRoutingTable" styleId="msisdnBasedRoutingTableId">
										<html:hidden name="msisdnBasedRoutingTableForm" styleId="pageNumber" property="pageNumber" />
										<html:hidden name="msisdnBasedRoutingTableForm" styleId="totalPages" property="totalPages" value="<%=strTotalPages%>" />
										<html:hidden name="msisdnBasedRoutingTableForm" styleId="totalRecords" property="totalRecords" value="<%=strTotalRecords%>" />
										<html:hidden name="msisdnBasedRoutingTableForm" styleId="action" property="action" />
													...
													<table cellSpacing="0" cellPadding="0" width="100%" border="0">
														...
														<tr>
															...
															<td align="left" class="labeltext" valign="top" width="80%">
																<html:text styleId="msisdnBasedRoutingTableName" property="msisdnBasedRoutingTableName" tabindex="1" size="30" maxlength="128" onkeyup="verifyName();" style="width:250px" />
														...
														<tr>
															...
															<td style="padding-left: 5px;">
																<input type="button" name="Search" width="5%"  onclick="validateSearch()" value="   Search   " class="light-btn" tabindex="5" />
																	...
																	<tr>
																		...
																		<tr>
																			<td class="btns-td"  colspan="2">
																				...
																				<logic:iterate id="msisdnRoutingTableBean" name="msisdnBasedRoutingTableForm" property="listMSISDNBasedRoutingTable" type="com.elitecore.elitesm.datamanager.diameter.msisdnbasedroutingtable.data.MSISDNBasedRoutingTableData">
																					<tr>
																						<td align="center" class="tblfirstcol">
																							<input type="checkbox" name="select" value="<bean:write name="msisdnRoutingTableBean" property="routingTableId"/>" />
																						</td>
