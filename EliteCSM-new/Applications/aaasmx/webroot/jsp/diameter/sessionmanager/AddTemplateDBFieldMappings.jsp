...
<%  		
	...
	DiameterSessionManagerMappingData diameterSessionManagerMappingData = diameterSessionManagerBLManager.getDiameterSessionManagerMappingDataById(defaultTemplateId);
											...
											<tr>
												<td>
													<table width="100%" cellspacing="0" cellpadding="0" border="0" id="dbFieldMappingTable<%=index%>" class="dbFieldMappingTable">
														...
														<tr>
															<% String dbFieldValue = (sessionManagerFieldMappingData.getDbFieldName() == null )?"":sessionManagerFieldMappingData.getDbFieldName(); %>
															<td class="allborder"><input  class="dbFieldName noborder" type="text" name="dbFieldName<%=index%>" id="dbFieldName<%=index%>" value="<%=dbFieldValue%>" old-value="<%=dbFieldValue%>" maxlength="1000" size="28" style="width:100%" /></td>
															<td class="tblrows"><input  class="referringAttribute noborder" type="text" name="referringAttribute<%=index%>"  id="referringAttribute<%=index%>" value="<%=(sessionManagerFieldMappingData.getReferringAttr() == null)?"":sessionManagerFieldMappingData.getReferringAttr()%>" maxlength="1000" size="28" style="width:100%" /></td>
															<td class="tblrows">
																<% if(sessionManagerFieldMappingData.getDataType() == 0){ %>
																		<select class="dataType" name="dataType<%=index%>" id="dataType<%=index%>" value="<%=sessionManagerFieldMappingData.getDataType()%>" size="1"  style="width: 100%;" selected="selected">
...
<table style="display:none;" id="dbFieldMappingTemplate<%=index%>">
	<tr>
		<td class="allborder"><input  autocomplete="off" class="dbFieldName noborder" type="text" name="dbFieldName<%=index%>" id="dbFieldName<%=index%>"  maxlength="1000" size="28" style="width:100%" /></td>
		<td class="tblrows"><input  class="referringAttribute noborder" type="text" name="referringAttribute<%=index%>"  id="referringAttribute<%=index%>" maxlength="1000" size="28" style="width:100%" /></td>
		<td class="tblrows">
			<select name="dataType<%=index%>" id="dataType<%=index%>" size="1"  style="width: 100%;">
				<option value="<%=Integer.toString(FieldMapping.STRING_TYPE)%>">String</option>
				<option value="<%=Integer.toString(FieldMapping.TIMESTAMP_TYPE)%>">Timestamp</option>
			</select>
		</td>
