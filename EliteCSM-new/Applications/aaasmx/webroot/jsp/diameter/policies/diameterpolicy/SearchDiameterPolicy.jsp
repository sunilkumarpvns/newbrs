...
<%@ page import="com.elitecore.elitesm.web.diameter.policies.diameterpolicy.forms.SearchDiameterPolicyForm"%>
...
function validateSearch(){
	document.searchDiameterPolicyForm.pageNumber.value = 1;
	document.searchDiameterPolicyForm.submit();
}
...
function navigate(direction, pageNumber ){
	document.searchDiameterPolicyForm.pageNumber.value = pageNumber;
	document.searchDiameterPolicyForm.submit();
}
...
function show(){
	document.miscDiameterPolicyForm.action.value = 'show';	
	...
	if(selectArray.length>0){
			...
			alert("Please select atleast one policy to Show.")
					...
					document.miscDiameterPolicyForm.submit();
			}
	}
	else{
...
function hide(){
	document.miscDiameterPolicyForm.action.value = 'hide';
...
function removeData(){
	document.miscDiameterPolicyForm.action.value = 'delete';
...
<%
     SearchDiameterPolicyForm searchDiameterPolicyForm = (SearchDiameterPolicyForm)request.getAttribute("searchDiameterPolicyForm");
     List lstDiameterPolicy = searchDiameterPolicyForm.getListDiameterPolicy();
     String strName = searchDiameterPolicyForm.getName();
     
     long pageNo = searchDiameterPolicyForm.getPageNumber();
     long totalPages = searchDiameterPolicyForm.getTotalPages();
     long totalRecord = searchDiameterPolicyForm.getTotalRecords();
	 int count=1;
     
     String strStatus = searchDiameterPolicyForm.getStatus();
	...
	<tr>
		...
		<td>
				...
				<tr>
					<td width="100%" class="box">
						<table cellpadding="0" cellspacing="0" border="0" width="100%">
							...
							<tr>
								<td class="table-header" colspan="5">SEARCH DIAMETER POLICY
								</td>
							</tr>
							...
							<tr>
								<td colspan="3">
									<table width="100%" name="c_tblCrossProductList"
										...
										<html:form action="/searchDiameterPolicy">
											<input type="hidden" name="c_strActionMode"
												id="c_strActionMode" value="102231105" />
											<html:hidden name="searchDiameterPolicyForm" styleId="action"
												property="action" />
											<html:hidden name="searchDiameterPolicyForm"
											...
											<tr>
												<td align="left" class="captiontext" valign="top"
													width="10%">
													<bean:message bundle="diameterResources"
													key="diameterpolicy.status" /> 
													<ec:elitehelp headerBundle="diameterResources" 
													text="diameterpolicy.status" header="diameterpolicy.status"/>
												</td>
												<td align="left" class="labeltext" valign="top" width="5%">
													<html:radio name="searchDiameterPolicyForm"
														styleId="status" property="status" value="Show"
														tabindex="2" />Show <html:radio
														name="searchDiameterPolicyForm" styleId="status"
											...
											<tr>
												...
												<td align="left" class="labeltext" valign="top" width="5%">
													<input type="button" name="Search" tabindex="5" width="5%"
													name="DiameterPolicyName"
													Onclick="validateSearch(document.searchDiameterForm,'list')"
													...
													value="   Create   "
													onclick="javascript:navigatePageWithStatus('initCreateDiameterPolicy.do','name');"
													class="light-btn">
												</td>
											</tr>
										</html:form>
		    ...
		    //System.out.println("Every time check the value of pageno and strname : "+pageNo+" str is :"+strName);
				if(searchDiameterPolicyForm.getAction()!=null && searchDiameterPolicyForm.getAction().equalsIgnoreCase(RadiusPolicyConstant.LISTACTION)){
	       	%>
										<html:form action="/miscDiameterPolicy">
											<html:hidden name="miscDiameterPolicyForm" styleId="action"
												property="action" />
											<html:hidden name="miscDiameterPolicyForm" styleId="name"
												property="name" value="<%=strName%>" />
											<html:hidden name="miscDiameterPolicyForm" styleId="status"
												property="status" value="<%=strStatus%>" />
											<html:hidden name="miscDiameterPolicyForm"
												styleId="pageNumber" property="pageNumber"
												value="<%=strPageNumber%>" />
											<html:hidden name="miscDiameterPolicyForm"
												styleId="totalPages" property="totalPages"
											...
											<tr>
												<td align="left" class="labeltext" colspan="5" valign="top">
													<table cellSpacing="0" cellPadding="0" width="100%"
														border="0">
														<tr>
															<td class="table-header" width="50%">DIAMETER POLICY
																LIST</td>
														...
														<tr>
															<td class="btns-td" valign="middle"><input
																...
																value="   Inactive   " class="light-btn"> <html:button
																	property="c_btnDelete" tabindex="9" onclick="removeData()"
																	value="   Delete   " styleClass="light-btn" /></td>
															<td class="btns-td" align="right">
																<% if(totalPages >= 1) { %> <% if(pageNo == 1){ %> <a
																href="searchDiameterPolicy.do?resultStatus=<%=strStatus%>&action=list&pageNo=<%= pageNo+1%>"><img
																...
																<a
																href="searchDiameterPolicy.do?resultStatus=<%=strStatus%>&action=list&pageNo=<%= totalPages+1%>"><img
																...
																<a
																href="searchDiameterPolicy.do?resultStatus=<%=strStatus%>&action=list&pageNo=<%=1%>"><img
																	...
																	<tr>
																		...
																		<td align="center" class="tblheader" valign="top"
																			width="40px"><bean:message
																				...
																				key="diameterpolicy.serialnumber" /></td>
																		<td align="left" class="tblheader" valign="top"
																			width="25%"><bean:message
																				bundle="diameterResources" key="diameterpolicy.name" /></td>
																		<td align="left" class="tblheader" valign="top"
																			width="*"><bean:message
																				bundle="diameterResources"
																				key="diameterpolicy.description" /></td>
																		<td align="left" class="tblheader" valign="top"
																			width="20%"><bean:message
																				bundle="diameterResources"
																				key="diameterpolicy.lastupdate" /></td>
																		<td align="center" class="tblheader" valign="top"
																			width="40px"><bean:message
																				bundle="diameterResources"
																				key="diameterpolicy.status" /></td>
																		<td align="center" class="tblheader" valign="top"
																			width="40px"><bean:message
																				bundle="diameterResources" key="diameterpolicy.edit" /></td>
																	</tr>
																	<%	if(lstDiameterPolicy!=null && lstDiameterPolicy.size()>0){%>
																	<logic:iterate id="diameterPolicyBean"
																		name="searchDiameterPolicyForm"
																		property="listDiameterPolicy"
																		type="com.elitecore.elitesm.datamanager.diameter.diameterpolicy.data.DiameterPolicyData">
																		<%	DiameterPolicyData sData = (DiameterPolicyData)lstDiameterPolicy.get(iIndex);
							Timestamp dtLastUpdate = sData.getLastModifiedDate();
						%>
																		<tr>
																			<td align="center" class="tblfirstcol"><input
																				type="checkbox" name="select"
																				value="<bean:write name="diameterPolicyBean" property="diameterPolicyId"/>" />
																			</td>
																			<td align="center" class="tblrows"><%=((pageNo-1)*pageSize)+count%></td>
																			<td align="left" class="tblrows"><a
																				href="<%=basePath%>/initViewDiameterPolicy.do?diameterPolicyId=<bean:write name="diameterPolicyBean" property="diameterPolicyId"/>"><bean:write
																						name="diameterPolicyBean" property="name" /></a></td>
																			<td align="left" class="tblrows"><%=EliteUtility.formatDescription(diameterPolicyBean.getDescription()) %>&nbsp;&nbsp;</td>
																			...
																			<td align="center" class="tblrows">
																				<%
									String CommonStatusValue = (String)org.apache.struts.util.RequestUtils.lookup(pageContext, "diameterPolicyBean", "commonStatusId", null);
								...
								%> <img src="<%=basePath%>/images/hide.jpg" border="0"> <% }else if(CommonStatusValue.equals(RadiusPolicyConstant.SHOW_STATUS_ID)){ %>
																			...
																			<td align="center" class="tblrows"><a
																				href="initUpdateDiameterPolicy.do?diameterPolicyId=<bean:write name="diameterPolicyBean" property="diameterPolicyId"/>">
