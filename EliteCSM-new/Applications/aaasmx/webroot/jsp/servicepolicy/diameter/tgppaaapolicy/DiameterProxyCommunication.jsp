<%@page import="com.elitecore.aaa.diameter.service.application.handlers.conf.DiameterSynchronousCommunicationHandlerData.ProxyMode"%>
...
<input type="hidden" value="DiameterProxyCommunication" name="handlerJsName" id="handlerJsName" />
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr>
					<td width="96%" lign="left" class="tbl-header-bold" valign="top">
						...
						<input type="text" id="handlerName_<%=widgetId%>" name="handlerName" class="handler-name-txt" size="30" value="<bean:message bundle='servicePolicyProperties' key='tgppaaapolicy.tgppaaapolicy.diameterproxy.title' />" onkeyup="expand(this);" onkeypress="keyPressedForHandler(event,this);" onload="expand(this);" disabled="disabled"/>
						<input type="hidden" id="diaProxyhiddenHandlerName_<%=widgetId%>" name="hiddenHandlerName" class="hidden-handler-name" value="<bean:message bundle='servicePolicyProperties' key='tgppaaapolicy.tgppaaapolicy.diameterproxy.title' />" />
						<span class="handler-type">[<bean:message bundle='servicePolicyProperties' key='tgppaaapolicy.tgppaaapolicy.diameterproxy.title' />]</span>
					</td>
							...
							<table width="100%" cellspacing="0" cellpadding="0" border="0" >
								<tr>
									...
									<td class="labeltext" width="*">
										<input class="priorityResultCodes" type="text" name="priorityResultCodes" id="priorityResultCodes_<%=widgetId%>"  pattern="[0-9,]*" title="Please enter a valid priority result code(Comma Seperated Digit Only)." style="width: 250px;" onkeypress="validateCommasNumber(event)" <%if("true".equals(request.getParameter("isViewPage").toString())){ %>  disabled=" disabled" <%}%> />
									</td>
									<td class="labeltext" width="10%">
										<bean:message bundle="servicePolicyProperties" key="tgppaaapolicy.tgppaaapolicy.proxymode" />
										<ec:elitehelp  header="tgppaaapolicy.tgppaaapolicy.proxymode" headerBundle="servicePolicyProperties" text="tgppaaapolicy.tgppaaapolicy.proxymode" ></ec:elitehelp>
									</td>
									<td class="labeltext" width="20%">
										<select name="proxyMode" id="proxyMode_<%=widgetId%>" <%if("true".equals(request.getParameter("isViewPage").toString())){ %>  disabled=" disabled" <%}%> >
											<% for(ProxyMode proxyMode : ProxyMode.values()){ %>
												<option value="<%=proxyMode.name()%>"><%=proxyMode.name()%></option>
											<% } %>
										</select>
									</td>
								</tr>
...
<table id="radiusProxyCommunicationTemplate_<%=widgetId%>" style="display: none">
	<tr>
		...
		<td class="tblrowspolicy" width="19%" valign="middle">
			<input class="noborder" id="script_<%=widgetId%>" type="text" name="script" style="width: 100%;" <%if("true".equals(request.getParameter("isViewPage").toString())){ %>  disabled=" disabled" <%}%> />
		</td>
