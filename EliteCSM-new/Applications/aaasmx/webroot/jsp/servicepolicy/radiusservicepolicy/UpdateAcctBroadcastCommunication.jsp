...
<script type="text/javascript">
	...
	$.each(formData, function(key,value){
		 ...
		 if(key == 'isHandlerEnabled'){
			 	if( value == "true" ){
			 		$('#toggle-acctbroadcasthandler_<%=widgetId%>').attr('checked', true);
			 		$('#toggle-acctbroadcasthandler_<%=widgetId%>').val('true');
			 	}else{
			 		$('#toggle-acctbroadcasthandler_<%=widgetId%>').attr('checked', false);
			 		$('#toggle-acctbroadcasthandler_<%=widgetId%>').val('false');
			 		var handlerObject=$('#toggle-acctbroadcasthandler_<%=widgetId%>').closest('table[class^="handler-class"]');
		...
		if( key == 'handlerName'){
			 if( value.length > 0 ){
				$('#acctBroadcastHandlerName'+'<%=widgetId%>').attr('size', value.length);
				$('#acctBroadcastHandlerName'+'<%=widgetId%>').val(value);
				$('#acctBroadcastCommucnicationHiddenHandlerName_'+'<%=widgetId%>').val(value);
			 }
	   	}
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr>
					<td width="96%" lign="left" class="tbl-header-bold" valign="top">
						<input type="text" id="acctBroadcastHandlerName<%=widgetId%>" name="handlerName" size="25" class="handler-name-txt" value="<bean:message bundle="servicePolicyProperties" key="servicepolicy.proxypolicy.broadcastcommunication" />" onkeyup="expand(this);" onkeypress="keyPressedForHandler(event,this);" onload="expand(this);" disabled="disabled"/>
						<input type="hidden" id="acctBroadcastCommucnicationHiddenHandlerName_<%=widgetId%>" name="hiddenHandlerName" class="hidden-handler-name" value="<bean:message bundle="servicePolicyProperties" key="servicepolicy.proxypolicy.broadcastcommunication" />" />
						<input type="hidden" id="acctBroadcastCommunicationHandlerType_<%=widgetId%>" name="handlerType" class="handlerType" value="BroadcastingCommunication" />
					</td>
					...
					<td width="1%" align="left" valign="middle" class="tbl-header-bold" style="padding-right: 2px;line-height: 9px;">
						<div class="switch">
						  <input id="toggle-acctbroadcasthandler_<%=widgetId%>" name="isHandlerEnabled" class="is-handler-enabled is-handler-enabled-round" type="checkbox" value="true" checked="checked" onclick="changeValueOfFlow(this);"/>
						  <label for="toggle-acctbroadcasthandler_<%=widgetId%>"></label>
						</div>
					</td>
