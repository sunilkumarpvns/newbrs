...
$.each(formData, function(key,value){
	 if(key == 'proxyCommunicationList'){
	 	...
	 	if( value == "true" ){
	 		$('#toggle-proxyhandler_<%=widgetId%>').attr('checked', true);
	 		$('#toggle-proxyhandler_<%=widgetId%>').val('true');
	 	}else{
	 		$('#toggle-proxyhandler_<%=widgetId%>').attr('checked', false);
	 		$('#toggle-proxyhandler_<%=widgetId%>').val('false');
	 		var handlerObject=$('#toggle-proxyhandler_<%=widgetId%>').closest('table[class^="handler-class"]');
		...
		if( value.length > 0 ){
			 $('#proxySequencialhandlerName'+'<%=widgetId%>').attr('size', value.length);
			 $('#proxySequencialhandlerName'+'<%=widgetId%>').val(value);
			 $('#proxyCommucnicationHiddenHandlerName_<%=widgetId%>').val(value);
		}
	}
});
</script>
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr>
					<td width="96%" lign="left" class="tbl-header-bold" valign="top">
						<input type="text" id="proxySequencialhandlerName<%=widgetId%>" name="handlerName" class="handler-name-txt" size="21" value="<bean:message bundle="servicePolicyProperties" key="servicepolicy.proxypolicy.proxycommunication" />" onkeyup="expand(this);" onkeypress="keyPressedForHandler(event,this);" onload="expand(this);" disabled="disabled"/>
					...
					<td width="1%" align="left" valign="middle" class="tbl-header-bold" style="padding-right: 2px;line-height: 9px;">
						<div class="switch">
						  <input id="toggle-proxyhandler_<%=widgetId%>" name="isHandlerEnabled" class="is-handler-enabled is-handler-enabled-round" type="checkbox" value="true" checked="checked" onclick="changeValueOfFlow(this);"/>
						  <label for="toggle-proxyhandler_<%=widgetId%>"></label>
						</div>
					</td>
					...
					<td width="1%" style="padding-right: 5px" class="tbl-header-bold" valign="middle">
						<img alt="Delete" class="delele_proxy" title="Delete" id="proxyDelete" src="<%=request.getContextPath()%>/images/delete_proxy.png" onclick="deleteHandler(this);" height="14" width="14" style="cursor: pointer;"/>
					</td>
				...
				<table name="tblmProxyCommunication" width="100%" border="0" style="background-color: white;" class="" cellspacing="0" cellpadding="0">
					<tr>
						<td align="left" class="captiontext left-border right-border" valign="top" id="button" style="padding-top: 10px;">
							<input type="button" onclick="addProxyPolicyRow('proxyCommunicationTemplate_<%=widgetId%>','proxyCommunicationTbl_<%=widgetId%>');" value=" Add Proxy Communication " class="light-btn proxy-com-btn" style="size: 140px" tabindex="3"> 
						</td>
					</tr>
					<tr>
