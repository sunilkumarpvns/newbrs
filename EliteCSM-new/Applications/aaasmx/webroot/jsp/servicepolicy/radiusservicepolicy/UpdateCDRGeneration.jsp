	...
	$.each(formData, function(key,value){
		...
		if( key == 'handlerName'){
			if( value.length > 0 ){
				 ...
				 $('#cdrGenHiddenHandlerName_<%=widgetId%>').val(value);
			}
		}
	});
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr>
					<td width="96%" align="left" class="tbl-header-bold" valign="top">
						<input type="text" id="cdrGenhandlerName<%=widgetId%>" name="handlerName" class="handler-name-txt" size="17" value="<bean:message bundle="servicePolicyProperties" key="servicepolicy.proxypolicy.cdrgeneration" />" onkeyup="expand(this);" onkeypress="keyPressedForHandler(event,this);" onload="expand(this);" disabled="disabled"/>
...
<table id="cdrTemplate_<%=widgetId%>" style="display: none">
	<tr>
		...
		<td class="tblrows" width="19%">
			<select name="primaryDriverId" id="primaryDriverId_<%=widgetId%>" class="noborder primaryDriverId" style="width:210px"  onchange="if (typeof(this.selectedIndex) != 'undefined') openDriverWizard(this.selectedIndex,this,mappingtblcdr_<%=widgetId%>);">
			</select>
		</td>
		<td class="tblrows" width="19%">
			<select name="secondaryDriverId" id="secondaryDriverId_<%=widgetId%>" class="noborder secondaryDriverId" style="width:210px" onchange="if (typeof(this.selectedIndex) != 'undefined') openDriverWizard(this.selectedIndex,this,mappingtblcdr_<%=widgetId%>);">
			</select>
		</td>
		<td class="tblrows" width="19%">
			<input class="noborder script scriptInstAutocomplete" id="script_<%=widgetId%>" type="text" name="script" style="width: 100%;" maxlength="2000"/>
		</td>
		<td class="tblrows" width="19%" >
