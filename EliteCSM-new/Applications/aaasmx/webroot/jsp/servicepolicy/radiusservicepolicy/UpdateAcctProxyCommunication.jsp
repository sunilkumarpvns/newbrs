...
$.each(formData, function(key,value){
	 ...
	 if( key == 'handlerName'){
		if( value.length > 0 ){
			 ...
			 $('#acctProxyCommucnicationHiddenHandlerName_'+'<%=widgetId%>').val(value);
		}
	}
});
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr>
					<td width="96%" lign="left" class="tbl-header-bold" valign="top">
						<input type="text" id="acctProxySequencialhandlerName<%=widgetId%>" name="handlerName" class="handler-name-txt" size="21" value="<bean:message bundle="servicePolicyProperties" key="servicepolicy.proxypolicy.proxycommunication" />" onkeyup="expand(this);" onkeypress="keyPressedForHandler(event,this);" onload="expand(this);" disabled="disabled"/>
						<input type="hidden" id="acctProxyCommucnicationHiddenHandlerName_<%=widgetId%>" name="hiddenHandlerName" class="hidden-handler-name" value="<bean:message bundle="servicePolicyProperties" key="servicepolicy.proxypolicy.proxycommunication" />" />
						<input type="hidden" id="acctProxyCommunicationHandlerType_<%=widgetId%>" name="handlerType" class="handlerType" value="ProxyCommunication" />
					</td>
					...
					<td width="1%" align="left" valign="middle" class="tbl-header-bold" style="padding-right: 2px;line-height: 9px;">
						<div class="switch">
						  <input id="toggle-acctproxyhandler_<%=widgetId%>" name="isHandlerEnabled" class="is-handler-enabled is-handler-enabled-round" type="checkbox" value="true" checked="checked" onclick="changeValueOfFlow(this);"/>
