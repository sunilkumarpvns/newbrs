...
<script src="<%=request.getContextPath()%>/js/servicepolicy/radius-service-policy-accounting.js"></script>
...
function getBroadcastServerOptions(){
	...
	<logic:iterate id="obj" name="createRadiusServicePolicyForm" property="acctESIList">
		var obj = {};
...
function reloadDriverList(){
	 var response;
	 var primaryServerData = "<option value='0'>--Select--</option>";
...
function openAcctDriverpopup(){
	openAcctDriverPopup('acctDriverPopup','acctDriverDataCheckBoxId',jAcctDriverInstacneIds,jAcctDriverName,'selectedAcctDriverIds');
}
...
var selectedElementForProfileDrivers = ['selectedCacheDriverIds','selectedAdditionalDriverIds','selecteddriverIds'];
...
function validate(){
	...
	$('.form_profilelookup').each(function(){
		authHandlerJson.push($(this).serializeObject());
    });
	...
	$('.form_proxycommunication').each(function(){
		...
		proxyTableObj.find('tr').each(function(){
		   ...
		   if(typeof $(this).find("input:checkbox").val() !== 'undefined'){
			   acceptOnTimeout=  $(this).find("input:checkbox").val();
		   }
	...
	$('table#pre-acct-plugin-mapping-table > tbody > tr').each(function(){
		   ...
		   var preAcctPluginNameTxt,preAcctPluginArgumentTxt;
		
		   if(typeof $(this).find("textarea[name='preAcctPluginArgumentTxt']").val() !== 'undefined'){
			   preAcctPluginArgumentTxt=  $(this).find("textarea[name='preAcctPluginArgumentTxt']").val();
		   }
		   
		   if(typeof $(this).find("input[name='preAcctPluginNameTxt']").val() !== 'undefined'){
			   preAcctPluginNameTxt=  $(this).find("input[name='preAcctPluginNameTxt']").val();
		   }
		   
		   if(  !isEmpty(preAcctPluginNameTxt) || !isEmpty(preAcctPluginArgumentTxt)){
			   prePluginDataMapping.push({'pluginName':preAcctPluginNameTxt,'pluginArgument':preAcctPluginArgumentTxt});
		   }
	});
	
	$('table#post-acct-plugin-mapping-table > tbody > tr').each(function(){
		   var postAcctPluginNameTxt,postAcctPluginArgumentTxt;
		
		   if(typeof $(this).find("textarea[name='postAcctPluginArgumentTxt']").val() !== 'undefined'){
			   postAcctPluginArgumentTxt=  $(this).find("textarea[name='postAcctPluginArgumentTxt']").val();
		   }
		   
		   if(typeof $(this).find("input[name='postAcctPluginNameTxt']").val() !== 'undefined'){
			   postAcctPluginNameTxt=  $(this).find("input[name='postAcctPluginNameTxt']").val();
		   }
		   
		   if( !isEmpty(postAcctPluginNameTxt) || !isEmpty(postAcctPluginArgumentTxt) ){
			   postPluginDataMapping.push({'pluginName':postAcctPluginNameTxt,'pluginArgument':postAcctPluginArgumentTxt});
		   }
	});
	...
	$('#acctHandlerJson').val(JSON.stringify(authHandlerJson));
...
function filterPluginData(pluginValue){
...
function getProxyCommunicationData(){
	var proxyList = [];
	<logic:iterate id="obj" name="createRadiusServicePolicyForm" property="acctESIList" type="com.elitecore.elitesm.datamanager.externalsystem.data.ExternalSystemInterfaceInstanceData">
...
<html:form action="/addRadiusServicePolicy" styleId="authForm">
	...
	<html:hidden property="acctHandlerJson" value="" styleId="acctHandlerJson"/>
				...
				<table cellpadding="0" cellspacing="0" border="0" width="100%">
					...
					<tr>
						<td colspan="3" align="left" style="padding-left: 290px;">
							...
							<input type="button" value=" Create " onclick="javascript:validate();" class="light-btn" tabindex="16" /> 
...
<table class="pre-acct-plugin-mapping-template" style="display:none">
	<tr>
		<td class="tblfirstcol" width="47.5%">
			<input type="text" name="preAcctPluginNameTxt" class="noborder" style="width:100%;" onfocus="setAutoCompleteDataAuthPluginData(this);"/>
		</td>	
		<td class="tblrows" width="47.5%">
			<textarea name="preAcctPluginArgumentTxt" rows="1" cols="1" style="width:100%;height: 19px;" class="noborder"></textarea>
		</td>	
...
<table class="post-acct-plugin-mapping-template" style="display:none">
	<tr>
		<td class="tblfirstcol" width="47.5%">
			<input type="text" name="postAcctPluginNameTxt" class="noborder" style="width:100%;" onfocus="setAutoCompleteDataAuthPluginData(this);"/>
		</td>	
		<td class="tblrows" width="47.5%">
			<textarea name="postAcctPluginArgumentTxt" rows="1" cols="1" style="width:100%;height: 19px;" class="noborder"></textarea>
		</td>	
		<td class="tblrows" width="5%" align="center">
			<span class='delete remove-proxy-server' onclick="deleteMe(this);"></span>&nbsp;
		</td>	
