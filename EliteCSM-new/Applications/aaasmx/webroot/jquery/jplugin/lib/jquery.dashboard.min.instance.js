(function (a) {
    a.fn.dashboardinstance = function (m) {
        var k = {};
        var f;
        var j;
        k.layout;
                ...
                if (!n.loaded) {
                    if (this.url != "" && this.url != null && typeof this.url != "undefined") {
                        ...
                        n.element.find("." + b.widgetContentClass).load(this.url, function (p, o, q) {
                            if (o == "error") {
                                n.element.find("." + b.widgetContentClass).html(b.widgetNotFoundHtml)
                            }
                            n.loaded = true;
        ...
        a("#" + k.id + " ." + b.widgetClass).live("widgetOpenFullScreen", function (n, p) {
            ...
            var idpr = $('.widgetcontent').parent().attr('id');
            $("#" + Id).find(".widgetcontent").find("#editConfigPage").val();
            if ($("#" + Id).find(".widgetcontent").find("#editConfigPage").val()) {
        ...
        a("." + b.widgetClass).live("widgetCloseFullScreen", function (n, p) {
            ...
            if ($("#" + Id).find(".widgetcontent").find("#editConfigPage").val()) {
                ...
                /*var widthOfDiv=$("#" +Id).find(".widgetcontent").outerWidth();
		
	var checkId=$("#" +Id).find(".widgetcontent").find("#realmStats").attr('id');
	var statsId=$("#" +Id).find(".widgetcontent").find("#statasticSummay2").attr('id');
	if(checkId == "realmStats"){
		alert('real'+widthOfDiv);
		$("#realmStats").width(widthOfDiv);
	}else if(statsId == "statasticSummay2"){
		alert('stat'+widthOfDiv);
		$("#statasticSummay2").width(widthOfDiv);
	}
	*/



                /*alert($("#realmStats").width());
		alert($("#statasticSummay2").width());*/
        ...
        a("#" + k.id + " ." + b.widgetClass).live("widgetEdit", function (n, p) {
            ...
            p.widget.setTitle(ide);
            if (Id == "widget1") {
                $.ajax({
                    url: "jsp/dashboardwidgets/aaadashboard/EditPrimaryServerWidget.jsp",
                    success: function (result) {
                        $("#" + Id).find(".widgetcontent").html(result);
                    }
                });
            }
            if (Id == "widget2") {
                $.ajax({
                    url: "jsp/dashboarddemo/editWidget2.jsp",
                    success: function (result) {
                        $("#" + Id).find(".widgetcontent").html(result);
                    }
                });
        ...
        k.element.live("dashboardOpenWidgetDialog", function () {
            ...
            a("#" + l.dialogId).find("." + l.widgetClass).empty();
           // alert(l+" : "+l.dialogId+" : "+l.widgetDirectoryUrl);
    ...
    a.fn.dashboard.defaults = {
        debuglevel: 3,
        json_data: {},
        loadingHtml: '<div class="loading"><img alt="Loading, please wait" src="images/loading1.gif" /></div>',
        emptyColumnHtml: "",
