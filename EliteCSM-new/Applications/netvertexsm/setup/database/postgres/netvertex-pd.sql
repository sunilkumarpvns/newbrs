...
COMMENT ON COLUMN TBLM_SYSTEM_PARAMETER.ALIAS IS 'Stores display name of parameter';
...
CREATE TABLE TBLM_PACKAGE
  (
    ...
    QUOTA_PROFILE_TYPE      VARCHAR(50) DEFAULT 'USAGE_METERING_BASED',
...
CREATE TABLE TBLM_DATA_SERVICE_TYPE
  (
    ...
    CONSTRAINT FK2_SERVICE_TYPE FOREIGN KEY (CREATED_BY) REFERENCES TBLM_STAFF (ID),
    CONSTRAINT FK3_SERVICE_TYPE FOREIGN KEY (MODIFIED_BY) REFERENCES TBLM_STAFF (ID)
  );
...
CREATE OR REPLACE FUNCTION trigger_fct_trg_subscription_history_del() RETURNS trigger AS $BODY$
BEGIN IF TG_OP = 'DELETE' THEN INSERT INTO TBLM_SUBSCRIPTION_HISTORY( SUBSCRIBER_ID , ADDON_ID , START_TIME , END_TIME , LAST_UPDATE_TIME , STATUS , SUBSCRIPTION_ID , PARENT_IDENTITY , SUBSCRIPTION_TIME , REJECT_REASON ) VALUES ( OLD.SUBSCRIBER_ID , OLD.ADDON_ID , OLD.START_TIME , OLD.END_TIME , OLD.LAST_UPDATE_TIME , OLD.STATUS , OLD.SUBSCRIPTION_ID , OLD.PARENT_IDENTITY , OLD.SUBSCRIPTION_TIME , OLD.PARAM1 ); END IF; RETURN OLD;
END
$BODY$
 LANGUAGE 'plpgsql';

CREATE TRIGGER trg_subscription_history_del
    AFTER DELETE ON tblm_subscription FOR EACH ROW
    EXECUTE PROCEDURE trigger_fct_trg_subscription_history_del();
...
CREATE TABLE TBLM_SERVER_INSTANCE (
  ...
  DIA_IS_ENABLE NUMERIC DEFAULT 1,
  DIA_URL VARCHAR(255),
  DIA_ORIGIN_HOST VARCHAR(255) DEFAULT 'netvertex.elitecore.com',
  DIA_ORIGIN_REALM VARCHAR(255) DEFAULT 'elitecore.com',
  RAD_IS_ENABLE NUMERIC DEFAULT 0,
  RAD_URL VARCHAR(255),
...
CREATE TABLE TBLM_SERVER_PROFILE (
  ...
  DIA_DUPLICATE_REQ_CHECK NUMERIC DEFAULT 0,
  ...
  RAD_DUPLICATE_REQ_CHECK NUMERIC DEFAULT 0,
...
INSERT INTO TBLM_SERVER_PROFILE ( ID, LOG_LEVEL ,ROLLING_TYPE , ROLLING_UNITS , MAX_ROLLED_UNITS ,
  DIA_QUEUE_SIZE , DIA_MINIMUM_THREADS , DIA_MAXIMUM_THREADS , DIA_DW_INTERVAL , DIA_DUPLICATE_REQ_CHECK ,
  DIA_DUPLICATE_REQ_PURGE_INTRVL , DIA_SESSION_CLEAN_UP_INTERVAL , DIA_SESSION_TIMEOUT ,
  RAD_QUEUE_SIZE , RAD_MINIMUM_THREADS , RAD_MAXIMUM_THREADS , RAD_DUPLICATE_REQ_CHECK , RAD_DUPLICATE_REQ_PURGE_INTRVL ,
  NOTIF_SRVC_EXEC_PERIOD , MAX_PARALLEL_EXECUTION , BATCH_SIZE ,
  PCRF_SERVICE_QUEUE_SIZE , PCRF_SERVICE_MAXIMUM_THREADS , PCRF_SERVICE_MINIMUM_THREADS , PCRF_SRVC_WRKR_THREAD_PRIORITY ,
  CREATED_DATE , CREATED_BY )
  values (1,'INFO',1,5,1,10000,5,10,60,0,15,86400, 86400, 10000,5, 10, 0,15,30,5,10,10000,10,5,7,CURRENT_TIMESTAMP,'1');
COMMIT;
