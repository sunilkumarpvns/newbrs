...
<%@page import="com.elitecore.netvertexsm.web.servermgr.server.form.LiveDictionaryData"%>
...
<%@ page	import="com.elitecore.netvertexsm.web.servermgr.server.form.ManageNetServerDictionaryForm"%>
<% 
...
<html:hidden name="manageNetServerDictionaryForm" styleId="" property="action" value="delete"/>
<html:hidden name="manageNetServerDictionaryForm" styleId="" property="netServerId" />
  		
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		
			<logic:equal name="manageNetServerDictionaryForm"
			   ...
			   <tr>
     				<td valign="top" align="right"> 
       					...
       					<tr>
       						<td>
       							<table width="100%" border="0" cellspacing="0" cellpadding="0">
   			          		<tr> 
								<td width="2%" class="tblheader-bold" colspan="3"><bean:message bundle="servermgrResources" key="servermgr.view.dictionarylist"/></td>
					  		</tr>
					  <%
	    				if(dictionaryMap != null && dictionaryMap.size() > 0){
 						 ...
 						 <tr>
			                ...
			                <tr>	
			                    ...
			                    <td align="left" class="tblheader" valign="top" width="1%"><bean:message bundle="servermgrResources" key="servermgr.serialnumber"/></td>
			                    <td align="left" class="tblheader" valign="top" width="90%"><bean:message bundle="servermgrResources" key="servermgr.name"/></td>
			                </tr>
			         		<%		if(dictionaryList != null && dictionaryList.size() > 0){
			         	    		int size= dictionaryList.size();
				         	%>
		                <logic:iterate id="liveDictionaryData" collection="<%=(List)dictionaryMap.get(key)%>" length="<%=Integer.toString(dictionaryList.size())%>" type="com.elitecore.netvertexsm.web.servermgr.server.form.LiveDictionaryData">
	 			           
	 			            <tr>
			                  <td align="left" class="tblfirstcol" valign="top">
			                  	<input type="checkbox" name="<%=key%>" id="<%=(iIndex+1) %>" value="<%=liveDictionaryData.getName()%>"/>
			                  </td>
						      <td align="left" class="tblrows" valign="top"><%=(iIndex+1) %></td>
			                  <td align="left" class="tblcol" valign="top">
			                  	<a href='<%=localBasePath1%>/initShowNetServerDictionary.do?fileName=<%=liveDictionaryData.getName()%>&fileGroup=<%=key%>&netServerId=<%=localNetServerInstanceData.getNetServerId()%>' onClick="return popup(this, '<%=liveDictionaryData.getName()%>');"><bean:write name="liveDictionaryData" property="name"/></a>
			                  </td>
			                </tr>       
			                <%iIndex+=1; %>         
						</logic:iterate>
				...
				<tr> 
					<td valign="middle" align="left">
	      				<input type="button" name="c_btnDelete"  id="c_btnDelete"  value=" Delete "  onclick="checkDictionarySelection()" class="light-btn">                   
	      				<input type="button" name="c_btnUpload" id="c_btnUpload"  onclick="javascript:location.href='<%=localBasePath1%>/uploadRadiusDictionary.do?netServerId=<bean:write name="manageNetServerDictionaryForm" property="netServerId"/>'"  value="Upload Dictionary"  class="light-btn">                   
