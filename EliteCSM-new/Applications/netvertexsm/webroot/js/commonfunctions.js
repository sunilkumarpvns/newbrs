...
*/
	...
	// Read -> checkbox row must have id="dataRow" , name="dataRow"   
	function onOffHighlightedRow(i,checkBox){
	...
	function onOffHighlightedRowUnique(i,checkBox,rowName){
		var dataRows = document.getElementsByName(rowName);
		if(checkBox.checked==true){
			dataRows[i].className='onHighlightRow';
		}else{
			dataRows[i].className='offHighlightRow';
		...
		try{
			if( parentCheckBox.checked == true) {
				...
				for (var i = 0; i < selectVars.length;i++){
					if(selectVars[i].disabled == false){ //checking whether checkbox enable/disable like in notification event
	...
	function filterTableRows(value,tableBodyID){
	    ...
	    var rows = $('tr');
	    var hRows = rows.filter('.onHighlightRow');	    
	    if (value == "") {
	        jo.show();
	        hRows.hide();
	        return;
	    ...
	    jo.hide();
	    jo = jo.filter('.offHighlightRow');
	    jo.filter(function (i, v) {
	        var $t = $(this);
	        var rowText = $t.text().toLowerCase();
	...
	function isNumberKey(evt){
	     if (evt.ctrlKey==true && (evt.which == '67' || event.which == '86')) {
	            return true;
	     }
	...
	//It will checked/unchecked all the child checkboxes, when parent checkbox is checked/unchecked.
	function  allCheck(parentCheckBox,nameOfBox,rowId){
		var dataRows = document.getElementsByName(rowId);
		try{
		 	if( parentCheckBox.checked == true) {
		 		var selectVars = document.getElementsByName(nameOfBox);
			 	for (var i = 0; i < selectVars.length;i++){
					selectVars[i].checked = true ;
					dataRows[i].className='onHighlightRow';
			 	}
