...
function isCommaSeperatedNumString(val){
	
	nre=/^([1-9]{1}[0-9]{0,7})+((,[1-9]{1}[0-9]{0,7}){0,1})+$/;
	var regexp  = new RegExp(nre);
	if(regexp.test(val))
	{
		return true;
...
function validateEmailAddress(emailId){
	...
	var emailRegEx = /^([0-9a-zA-Z]+([_.-]?[0-9a-zA-Z]+)*@[0-9a-zA-Z]+[0-9a-zA-Z]*(.)+[a-zA-Z]+)+$/;
...
function validateMobileNumber(valObj){
...
function checkDescriptionLength(limitField,limit){
  if(limitCheck(limitField,limit)){
       return true;
   }else{
    	alert("Description should not greater than "+limit+ "character.");
	    return false;
   }
...
function limitCheck(limitField,limit){
   ...
   var enteredStringLen = field.length;
...
/**  for (+)Ve integer numbers including 0
 *     Values  Result
 *     100      true
 *     100.00   false
 *     100.12   false
...
function isIntegerNumber(val){
	//includes all integer i.e {...,-2,-1,0,1,2...}
	if(val==null){
		return false;
	}
	var nre= /^\s*(\+|-)?\d+\s*$/;
...
function isIntegerRange(val, startVal, endVal){
	//includes withing range. if starval = -2, endval=3 then {-2,-1,0,1,2,3}
	if(val==null){
		return false;
	}
	var nre= /^\s*(\+|-)?\d+\s*$/;
...
//to check for positive numeric fields
/*   value   result
 *   100     false  
 *   100.00   true 
 *   -1000    true
...
/* to test for postive decimal numbers
* 
*    values ---- result 
*      null ----    false
*       "" ----    false
...
* 
* */
function isPostiveDecimalNumber(val){
...
function checkAddonPrice(val,precision,scale) {
	if(val == null || val.trim().length == 0){
		return false;
	}
	var temp = val.indexOf(".");
	var len = val.length;
	var integerDigitValue = precision - scale;
	
	if(temp == -1){
		if(len <= integerDigitValue){
			return true;
		}else{
			return false;
		}
	...
	var priceLength = val.split(".");
	if(priceLength[0].length <= integerDigitValue){
		return true;
	}else{
		return false;
	}
...
function verifyValidFormat(type){
	 var allowedFormatRegex = "^[a-zA-Z0-9-_%.:]*$"; 
	 var message = 'false'; 
	 
	 if((type.isSpaceAllowed != 'undefined' && type.isSpaceAllowed == 'yes') || (type.isMenuCall != 'undefined' && type.isMenuCall == 'yes')){
		 message = 'invalid';
		 allowedFormatRegex="^[a-zA-Z0-9- _%.:]*$"; //with space
		 
	 }
	 if(type.isPolicyManagementSubModule != 'undefined' &&  type.isPolicyManagementSubModule == 'yes'){
		 message = 'invalidPolicy';
		 allowedFormatRegex="^[a-zA-Z0-9 _%.:`~!@$^*()+|:;.,{}\\[\\]]*$"; //with space and some special characters 
	 }
	 var pattern = new RegExp(allowedFormatRegex);
	 if(pattern.test(type.searchName) == true){
		 return 'true';
	 }else{
		 return message;
	 }
