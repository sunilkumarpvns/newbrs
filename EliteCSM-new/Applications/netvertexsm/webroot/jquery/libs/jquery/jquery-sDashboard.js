		...
		$.widget("mn.sDashboard", {
				...
				this.element.sortable({
					...
					update : function(event, ui) {
						...
						for ( i = 0; i < sortOrderArray.length; i++) {
							var widgetContent = that._getWidgetContentForId(parseInt(sortOrderArray[i]), that);
							sortedDefinitions.push(widgetContent);
							sortedWidgetIds.push(widgetContent.widgetId);
						}
			...
			_bindEvents : function() {
				...
				this.element.on("click",".sDashboardWidgetHeader span.ui-icon.ui-icon-circle-plus",function(e){
					...
					var widgetDefinition = self._getWidgetContentForId(parseInt(widgetListItem.attr("id")), self);
					...
					if (widgetDefinition.widgetGraphType === "chart") {
				...
				this.element.on("click",".sDashboardWidgetHeader span.ui-icon.ui-icon-arrowrefresh-1-e",function(e){
					...
					var widgetDefinition = self._getWidgetContentForId(parseInt(widgetId), self);
					var refreshedData = widgetDefinition.refreshCallBack.apply(self,[widgetId]);
					widgetDefinition.widgetContent.data = refreshedData.data;
					if(widgetDefinition.widgetGraphType === 'chart'){
						self._renderChart(widgetDefinition);
					}else if(widgetDefinition.widgetGraphType === 'table'){
				...
				// edit button click handler to append edit box 
				this.element.on("click",".sDashboardWidgetHeader span.ui-icon.ui-icon-circle-triangle-s",function(e){
					var div = $(e.currentTarget).parents("li:first").find("div.sDashboardWidgetContent");
					var editdiv = $(e.currentTarget).parents("li:first").find("#editBoxDiv1");
					editdiv.toggle("blind");
					div.toggle("blind");
					$(e.currentTarget).toggleClass("ui-icon-circle-triangle-n");
				});
				
				
				
				this.element.on("click","#editBoxSubmit",function(e){
					var widgetContent = $(e.currentTarget).parents("li:first").find("div.sDashboardWidgetContent");
					var widgetHeader=$(e.currentTarget).parents("li div:last-child");
					widgetHeader.toggleClass("ui-icon-circle-triangle-s");
					widgetContent.show("slow");
				...
				//delete widget by clicking the 'x' icon on the widget
				 this.element.on("click",".sDashboardWidgetHeader span.ui-icon.ui-icon-circle-close",function(e){
					var widget = $(e.currentTarget).parents("li:first");
					var widgetId = widget.attr("id");
					deleteWidgetFromDashBoard(widgetId);
					//show hide effect
				...
				this.element.on("click",".sDashboardWidgetContent table.sDashboardTableView tbody tr",function(e){
					...
					if (selectedRow.length > 0) {
						...
						var widgetDefinition = self._getWidgetContentForId(parseInt(selectedWidgetId), self);
			...
			_constructWidget : function(widgetDefinition) {
				...
				if(widgetDefinition.hasOwnProperty("enableEdit") && widgetDefinition.enableEdit){
					var editButton = $('<span title="Edit" class="ui-icon ui-icon-circle-triangle-s"></span>');
						//add edit button
				...
				if (widgetDefinition.widgetGraphType === 'table') {
					var tableDef = {						
					...
					var dataTable = $('<table cellpadding="0" cellspacing="0" border="0" class="display sDashboardTableView table table-bordered" style="width: 100%;"></table>').dataTable(tableDef);
					widgetContent.append(dataTable);
				} else if (widgetDefinition.widgetGraphType === 'chart') {
				...
				if(widgetDefinition.hasOwnProperty("enableEdit") && widgetDefinition.enableEdit){
					var editBoxDiv=$('<div class="sDashboardWidgetEditBox" id="editBoxDiv1">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;title&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="title" id="title" name="title" maxlength="30" size="20" /><br>' +
							'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" id="editBoxSubmit" name="editBoxSubmit" value="submit" /></div>');
					widgetContainer.append(editBoxDiv);
					}
