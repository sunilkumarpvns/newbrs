package com.elitecore.coreeap.packet.types.akaprime;
...
public class AkaPrimeEapType extends EAPType {	
	...
	public AkaPrimeEapType() {
		super(EapTypeConstants.AKA_PRIME.typeId);
		this.akaMessageType = 0;
	}

	public AkaPrimeEapType(byte[] byteArray) throws InvalidEAPTypeException {
		super(byteArray);
		if(super.getType() == EapTypeConstants.AKA_PRIME.typeId){			
			...
			throw new InvalidEAPTypeException("EAP Type : not AKA-PRIME");
		}
	}
	...
	public Object clone() throws CloneNotSupportedException {
		AkaPrimeEapType newObject = (AkaPrimeEapType)super.clone();
		if (this.akaDataBytes!=null){
	...
	public List<byte[]> getAkaAttributes(){
		...
		if(this.akaDataBytes != null)
		{
			...
			while(lengthRead < lengthOfAllRecords){				
				...
				LogManager.getLogger().debug("Aka-Prime EAP Type ", "Aka Data bytes : " + TLSUtility.bytesToHex(this.akaDataBytes));
				LogManager.getLogger().debug("Aka-Prime EAP Type", "Length : " + length);
				LogManager.getLogger().debug("Aka-Prime EAP Type", "Aka Data bytes Length : " + this.akaDataBytes.length);
				LogManager.getLogger().debug("Aka-Prime EAP Type", "Length Read : " + lengthRead);				
