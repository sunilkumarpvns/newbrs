...
public class AKAMethodStateMachine extends BaseAKAMethodStateMachine {

	private static final String MODULE = "AKA STATE MACHINE";
	
	public AKAMethodStateMachine(
	...
	public IEnum processRequest(AAAEapRespData aaaEapRespData,ICustomerAccountInfoProvider provider) throws EAPException {
		LogManager.getLogger().trace(getModuleName(), "AKA Packet Processing in State :" + getCurrentState());		
		setSuccess(false);
		setFailure(false);
		setDone(false);
		...
		handleEvent(event, aaaEapRespData,provider);
			...
			LogManager.getLogger().warn(MODULE, "Failed to handle EAP-AKA event: " + event);
			setSuccess(false);
			setFailure(true);
			setDone(true);
			setFailureReason(EAPFailureReasonConstants.AKA_FAILURE);
		}
		LogManager.getLogger().info(getModuleName(), "AKA Packet has been processed");
		return getCurrentEvent();
	}
	@Override
	public AAAEapRespData handleEvent(IEnum event, AAAEapRespData aaaEapRespData, ICustomerAccountInfoProvider provider)
			throws EAPException {
		LogManager.getLogger().trace(getModuleName(), "Handle - AKA Event :"+ event);
		IEnum[] actionList = getActionList(event);
		if(actionList != null){
			LogManager.getLogger().trace(getModuleName(), "AKA Actions for Event :"+ event + " = " + actionList);
			for(int iCounter=0; iCounter < actionList.length ; iCounter ++){
				...
				case GenerateResponse:
					...
					break;
				case Resynchronize:
					resynchronizeRequest(aaaEapRespData,provider);
					break;
				case ProcessResponseIdentity:
					processResponseIdentity(aaaEapRespData, provider);
					break;
				case Validate_Response:
					validateAKAResponse(aaaEapRespData,provider);
				}
			}
		}else{
			LogManager.getLogger().trace(getModuleName(), "No AKA Actions for Event :"+ event);
		}
		return null;
	}

	public boolean check(AAAEapRespData aaaEapRespData) {
		parseAKAResp(aaaEapRespData);
		return isValidRequest();
	}

	public int getMethodCode() {	
		return EapTypeConstants.AKA.typeId;
	}
	...
	public void postProcess(AAAEapRespData aaaEapRespData,ICustomerAccountInfoProvider provider)throws EAPException{
		...
		if(akaEapType.getAkaMessageType() == SIMMessageTypeConstants.AKA_IDENTITY.Id){
			...
			AkaEapType receivedAkaEapType = (AkaEapType)aaaEapRespData.getEapReqPacket().getEAPType();
...
//			nonce_Mt = TLSUtility.HexToBytes("0x4545ef027b8d04004645ef027b8d0400");
			...
			List<ISIMAttribute> akaAttrList = ((AkaEapType)eapPacket.getEAPType()).getAkaAttrList();
			...
			((AkaEapType)eapPacket.getEAPType()).setAkaAttrList(akaAttrList);
			try {
