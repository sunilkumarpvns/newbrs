...
public class AddOnFactoryTest {
	...
	public class DeploymentModeIsPCC {
		...
		private PkgData addOnPackageData;
		private DeploymentMode deploymentMode = DeploymentMode.PCC;

		@Before
		public void setUp() {
			addOnPackageData = PkgDataBuilder.newBasePackageWithRnCProfileAndPCCProfile(false);
			addOnFactory = new AddOnFactory(qoSProfileFactory, usageNotificationSchemeFactory,
		...
		public class FailWhenQuotaProfileTypeIsNotRnCBased {
			...
			public void QuotaProfileTypeIsUMBased() {
				addOnPackageData.setQuotaProfileType(QuotaProfileType.USAGE_METERING_BASED);
				AddOn addOn = createPackage(addOnPackageData);

				assertEquals(PolicyStatus.FAILURE, addOn.getStatus());
				assertThat(addOn.getFailReason(), containsString("Quota profile type: "
			...
			public void QuotaProfileTypeIsSyBased() {
				addOnPackageData.setQuotaProfileType(QuotaProfileType.SY_COUNTER_BASED);
				AddOn addOn = createPackage(addOnPackageData);
		...
		public class FailWhenQuotaProfileIsRnCBasedAnd {
			...
			@Before
			public void setUp() {
				addOnPackageData.setQuotaProfileType(QuotaProfileType.RnC_BASED);
			}

			@Test
			public void QuotaConfigured_And_QoSDetailsNotConfigured() {
				removeQoSDetailsFromPackage(addOnPackageData);

				AddOn addOn = createPackage(addOnPackageData);

				assertEquals(PolicyStatus.FAILURE, addOn.getStatus());
				assertThat(addOn.getFailReason(), containsString("QoS detail is not configured"));
			}

			@Test
			public void QuotaNotConfigured_And_QoSDetailsConfigured() {

				removeRnCProfilePCCPRofile(addOnPackageData);

				AddOn addOn = createPackage(addOnPackageData);

				assertEquals(PolicyStatus.FAILURE, addOn.getStatus());
				assertThat(addOn.getFailReason(), containsString("RnC Quota Profile/Rate Card is not attached with PCC profile"));
			}
			...
			public void RateCardQuotaConfigured_And_QoSDetailsNotConfigured() {
				...
				AddOn addOn = createPackage(basePackageData);
				assertEquals(PolicyStatus.FAILURE, addOn.getStatus());
				assertThat(addOn.getFailReason(), containsString("QoS detail is not configured"));
			}
			...
			public void QuotaNotConfigured_And_QoSDetailsNotConfigured() {
				removeQoSDetailsFromPackage(addOnPackageData);
				removeRnCProfilePCCPRofile(addOnPackageData);
				AddOn addOnPackage = createPackage(addOnPackageData);

				assertEquals(PolicyStatus.FAILURE, addOnPackage.getStatus());
				assertThat(addOnPackage.getFailReason(), containsString("QoS detail is not configured"));
				assertThat(addOnPackage.getFailReason(), containsString("RnC Quota Profile/Rate Card is not attached with PCC profile"));
			}

			@Test
			public void RateCardNotConfigured_And_QoSDetailsNotConfigured() {
				PkgData addOnPackageData = PkgDataBuilder.newBasePackageWithRateCardAndPCCProfile(false);
				addOnPackageData.setQuotaProfileType(QuotaProfileType.RnC_BASED);

				removeRateCardFromQoS(addOnPackageData);
				removeRateCardFromQoS(addOnPackageData);
		...
		public class SuccessWhenQuotaProfileTypeIsRnCBasedAnd {
			...
			public void Quota_and_QoSDetailsConfigured() {
				AddOn addOn = createPackage(addOnPackageData);
				assertEquals(PolicyStatus.SUCCESS, addOn.getStatus());
			}
		}
	}
	...
	public class DeploymentTypePCRF {
		...
		public class SuccessWhenQuotaProfileTypeIsUMBasedAnd {
			...
			public void QoSDetailConfigured() {
				AddOn basePackage = createPackage(basePackageData);
	...
	public class DeploymentTypeOCS {
		...
		public class FailWhen {
			...
			public void qosDetailsConfigured() {
				...
				assertEquals(PolicyStatus.FAILURE, addOn.getStatus());
				assertThat(addOn.getFailReason(), containsString("QoS detail is configured. Deployment mode is " + deploymentMode));
			}
		}
	...
	private AddOn createPackage(PkgData addonData) {
		return addOnFactory.createAddOn(addonData, PkgStatus.ACTIVE, Collections.emptyList());
	}
}