...
public class FunctionFloor extends AbstractIntegerFunctionExpression {
	...
	public long getLongValue(ValueProvider valueProvider)throws InvalidTypeCastException,IllegalArgumentException, MissingIdentifierException {
		if(argumentList.size() != 1)
			throw new IllegalArgumentException("Number of parameter mismatch ,floor function has 1 arguments 1)value ");
		String valuestr = argumentList.get(0).getStringValue(valueProvider);
		
		try{
			return (long) Math.floor(Double.parseDouble(valuestr));
