 ...
 *
 */
class AsynchronousSave implements SaveOperation {
	
	private static final String MODULE = "ASYNCHRONOUS-SAVE-OPERATION";
	
	@Nonnull
	protected final BatchDataSink batchedOperationTask;

	public AsynchronousSave(BatchDataSink batchedOperationTask) {
		this.batchedOperationTask = batchedOperationTask;
	}

	@Override
	public int execute(SessionData sessionData) {
		boolean addedToBatch = batchedOperationTask.addBatchData(new BatchUpdateData(sessionData,BatchUpdateData.INSERT));
		if (addedToBatch == false) {
			if (LogManager.getLogger().isLogLevel(LogLevel.WARN)) {
				LogManager.getLogger().warn(MODULE, "Queue is full or DB failure, so dropping save operation for ID: " + sessionData.getSessionId());
			}
