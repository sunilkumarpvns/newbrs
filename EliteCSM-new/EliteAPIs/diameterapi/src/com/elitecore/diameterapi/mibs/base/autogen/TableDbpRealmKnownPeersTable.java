...
/**
 * The class is used for implementing the "DbpRealmKnownPeersTable" group.
 * The group is defined with the following oid: 1.3.6.1.2.1.119.1.5.1.
 */
public class TableDbpRealmKnownPeersTable extends SnmpTableSupport implements Serializable {

    /**
     * Constructor for the table. Initialize metadata for "TableDbpRealmKnownPeersTable".
     ...
     */
    public TableDbpRealmKnownPeersTable(SnmpMib myMib) {
        super(myMib);
    }
     ...
     */
    public TableDbpRealmKnownPeersTable(SnmpMib myMib, MBeanServer server) {
        this(myMib);
        this.server = server;
    }
    ...
    public void createNewEntry(SnmpMibSubRequest req, SnmpOid rowOid,
        ...
        try  {
            ...
            // the returned object must implement the "DbpRealmKnownPeersEntryMBean"
            // interface.
            //
            final Object entry =
                 createDbpRealmKnownPeersEntryMBean(req, rowOid, depth, objname, meta,
                    oid0.toLong(),
                    oid1.toLong());
            if (server != null) {
    ...
    protected SnmpMibTable getRegisteredTableMeta(SnmpMib mib)  {
        return mib.getRegisteredTableMeta("DbpRealmKnownPeersTable");
    }
    ...
    public synchronized void addEntry(DbpRealmKnownPeersEntryMBean entry)
        throws SnmpStatusException {
    ...
    public synchronized void addEntry(DbpRealmKnownPeersEntryMBean entry, ObjectName name)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public synchronized DbpRealmKnownPeersEntryMBean[] getEntries() {
        Object[] array = getBasicEntries();
        DbpRealmKnownPeersEntryMBean[] result = new DbpRealmKnownPeersEntryMBean[array.length];
    ...
    public void removeEntry(DbpRealmKnownPeersEntryMBean entry)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public ObjectName buildNameFromIndex(SnmpIndex index)
        ...
        try  {
            ...
            String _keyDbpRealmMessageRouteIndex = oid.toLong().toString();
            oid = (SnmpOid) v.elementAt(1);
            String _keyDbpRealmKnownPeersIndex = oid.toLong().toString();
            return new ObjectName("TableDbpRealmKnownPeersTable:name=com.elitecore.diameterapi.mibs.base.autogen.DbpRealmKnownPeersEntry" + ",DbpRealmMessageRouteIndex=" + _keyDbpRealmMessageRouteIndex + ",DbpRealmKnownPeersIndex=" + _keyDbpRealmKnownPeersIndex);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.snmpRspWrongValue);
        } catch(Exception e) {
            throw new SnmpStatusException(e.getMessage());
    ...
    /**
     * Build index for "DbpRealmKnownPeersEntry".
     */
    public SnmpIndex buildSnmpIndex(DbpRealmKnownPeersEntryMBean entry)
        throws SnmpStatusException {
        SnmpOid[] oids = new SnmpOid[2];
        SnmpValue val = null;
        val = new SnmpGauge(entry.getDbpRealmMessageRouteIndex());
        oids[0] = val.toOid();
        val = new SnmpGauge(entry.getDbpRealmKnownPeersIndex());
        oids[1] = val.toOid();
     ...
     * Build index for "DbpRealmKnownPeersEntry".
     */
    public SnmpOid buildOidFromIndexVal(Long aDbpRealmMessageRouteIndex, Long aDbpRealmKnownPeersIndex)
        throws SnmpStatusException  {
        SnmpOid oid = new SnmpOid();
        try {
            SnmpGauge.appendToOid(new SnmpGauge(aDbpRealmMessageRouteIndex).toOid(), oid);
            SnmpGauge.appendToOid(new SnmpGauge(aDbpRealmKnownPeersIndex).toOid(), oid);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.noSuchInstance);
        }
        return oid;
    ...
    /**
     * Factory method for "DbpRealmKnownPeersEntry" entry MBean class.
     * 
     * You can redefine this method if you need to replace the default
     * generated MBean class with your own customized class.
     * 
     * @return An instance of the MBean class generated for the
     *         "DbpRealmKnownPeersEntry" conceptual row.
     * 
     * Note that when using standard metadata,
     * the returned object must implement the "DbpRealmKnownPeersEntryMBean"
     * interface.
    ...
    public Object createDbpRealmKnownPeersEntryMBean(SnmpMibSubRequest req,
                SnmpOid rowOid, int depth, ObjectName entryObjName,
                SnmpMibTable meta, Long  aDbpRealmMessageRouteIndex, Long  aDbpRealmKnownPeersIndex)
            throws SnmpStatusException  {
        ...
        // interface.
        //
        DbpRealmKnownPeersEntry entry = new DbpRealmKnownPeersEntry(theMib);
        entry.DbpRealmMessageRouteIndex = aDbpRealmMessageRouteIndex;
        entry.DbpRealmKnownPeersIndex = aDbpRealmKnownPeersIndex;
        return entry;
    }
