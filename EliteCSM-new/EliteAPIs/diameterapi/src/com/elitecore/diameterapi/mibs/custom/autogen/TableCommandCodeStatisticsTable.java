package com.elitecore.diameterapi.mibs.custom.autogen;

//
// Generated by mibgen version 5.1 (03/08/07) when compiling DIAMETER-STACK-MIB.
//

// java imports
//
import java.io.Serializable;
import java.util.Vector;

import javax.management.MBeanServer;
import javax.management.ObjectName;

import com.sun.management.snmp.SnmpGauge;
import com.sun.management.snmp.SnmpOid;
import com.sun.management.snmp.SnmpStatusException;
import com.sun.management.snmp.SnmpValue;
import com.sun.management.snmp.agent.SnmpIndex;
import com.sun.management.snmp.agent.SnmpMib;
import com.sun.management.snmp.agent.SnmpMibSubRequest;
import com.sun.management.snmp.agent.SnmpMibTable;
import com.sun.management.snmp.agent.SnmpTableSupport;
// jmx imports
//
// jdmk imports
//

/**
 * The class is used for implementing the "CommandCodeStatisticsTable" group.
 * The group is defined with the following oid: 1.3.6.1.4.1.21067.5.102.3.2.1.
 */
public class TableCommandCodeStatisticsTable extends SnmpTableSupport implements Serializable {

    /**
     * Constructor for the table. Initialize metadata for "TableCommandCodeStatisticsTable".
     * The reference on the MBean server is not updated so the entries created through an SNMP SET will not be registered in Java DMK.
     */
    public TableCommandCodeStatisticsTable(SnmpMib myMib) {
        super(myMib);
    }

    /**
     * Constructor for the table. Initialize metadata for "TableCommandCodeStatisticsTable".
     * The reference on the MBean server is updated so the entries created through an SNMP SET will be AUTOMATICALLY REGISTERED in Java DMK.
     */
    public TableCommandCodeStatisticsTable(SnmpMib myMib, MBeanServer server) {
        this(myMib);
        this.server = server;
    }


    // ------------------------------------------------------------
    // 
    // Implements the "createNewEntry" method defined in "SnmpTableSupport".
    // See the "SnmpTableSupport" Javadoc API for more details.
    // 
    // ------------------------------------------------------------

    public void createNewEntry(SnmpMibSubRequest req, SnmpOid rowOid,
                 int depth, SnmpMibTable meta)
        throws SnmpStatusException {
        final SnmpIndex index = buildSnmpIndex(rowOid);
        final Vector v = index.getComponents();
        SnmpOid oid;
        try  {
            final SnmpOid oid0 = (SnmpOid) v.elementAt(0);
            final SnmpOid oid1 = (SnmpOid) v.elementAt(1);
            final SnmpOid oid2 = (SnmpOid) v.elementAt(2);
            ObjectName objname = null;
            if (server != null)
                objname = buildNameFromIndex( index );

            // Note that when using standard metadata,
            // the returned object must implement the "CommandCodeStatisticsEntryMBean"
            // interface.
            //
            final Object entry =
                 createCommandCodeStatisticsEntryMBean(req, rowOid, depth, objname, meta,
                    oid0.toLong(),
                    oid1.toLong(),
                    oid2.toLong());
            if (server != null) {
                server.registerMBean(entry, objname);
            }
            meta.addEntry(rowOid,objname,entry);
        } catch(SnmpStatusException e) {
            throw e;
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.snmpRspWrongValue);
        } catch(Exception e) {
            throw new SnmpStatusException(e.getMessage());
        }
    }



    // ------------------------------------------------------------
    // 
    // Implements the "getRegisteredTableMeta" method defined in "SnmpTableSupport".
    // See the "SnmpTableSupport" Javadoc API for more details.
    // 
    // ------------------------------------------------------------

    protected SnmpMibTable getRegisteredTableMeta(SnmpMib mib)  {
        return mib.getRegisteredTableMeta("CommandCodeStatisticsTable");
    }


    // ------------------------------------------------------------
    // 
    // Implements the "removeEntryCb" method defined in "SnmpTableSupport".
    // See the "SnmpTableSupport" Javadoc API for more details.
    // 
    // ------------------------------------------------------------

    public void removeEntryCb(int pos, SnmpOid row, ObjectName name,
                Object entry, SnmpMibTable meta)
            throws SnmpStatusException {
        try  {
            super.removeEntryCb(pos,row,name,entry,meta);
            if (server != null && name != null)
                server.unregisterMBean(name);
        } catch (Exception x) { }
    }


    /**
     * Add a new entry to the table.
     * 
     * If the associated metadata requires ObjectNames
     * a new ObjectName will be generated using "buildNameFromIndex".
     * 
     * This method calls "addEntry" from "SnmpTableSupport".
     * See the "SnmpTableSupport" Javadoc API for more details.
     * 
     **/

    public synchronized void addEntry(CommandCodeStatisticsEntryMBean entry)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
        super.addEntry(index, (Object) entry);
    }


    /**
     * Add a new entry to the table.
     * 
     * This method calls "addEntry" from "SnmpTableSupport".
     * See the "SnmpTableSupport" Javadoc API for more details.
     * 
     **/

    public synchronized void addEntry(CommandCodeStatisticsEntryMBean entry, ObjectName name)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
        super.addEntry(index, name, (Object) entry);
    }


    /**
     * Return the entries stored in the table.
     * 
     * This method calls "getBasicEntries" from "SnmpTableSupport".
     * See the "SnmpTableSupport" Javadoc API for more details.
     * 
     **/

    public synchronized CommandCodeStatisticsEntryMBean[] getEntries() {
        Object[] array = getBasicEntries();
        CommandCodeStatisticsEntryMBean[] result = new CommandCodeStatisticsEntryMBean[array.length];
        java.lang.System.arraycopy(array,0, result,0, array.length);
        return result;
    }


    /**
     * Remove the specified entry from the table.
     * 
     * This method calls "removeEntry" from "SnmpTableSupport".
     * See the "SnmpTableSupport" Javadoc API for more details.
     * 
     **/

    public void removeEntry(CommandCodeStatisticsEntryMBean entry)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
        super.removeEntry(index, entry) ;
    }


    // ------------------------------------------------------------
    // 
    // Implements the "buildNameFromIndex" method defined in "SnmpTableSupport".
    // See the "SnmpTableSupport" Javadoc API for more details.
    // 
    // ------------------------------------------------------------


    public ObjectName buildNameFromIndex(SnmpIndex index)
        throws SnmpStatusException {
        Vector v = index.getComponents();
        SnmpOid oid;
        try  {
            oid = (SnmpOid) v.elementAt(0);
            String _keyPeerIndex = oid.toLong().toString();
            oid = (SnmpOid) v.elementAt(1);
            String _keyApplicationID = oid.toLong().toString();
            oid = (SnmpOid) v.elementAt(2);
            String _keyCommandCode = oid.toLong().toString();
            return new ObjectName("TableCommandCodeStatisticsTable:name=com.elitecore.diameterapi.mibs.custom.autogen.CommandCodeStatisticsEntry" + ",PeerIndex=" + _keyPeerIndex + ",ApplicationID=" + _keyApplicationID + ",CommandCode=" + _keyCommandCode);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.snmpRspWrongValue);
        } catch(Exception e) {
            throw new SnmpStatusException(e.getMessage());
        }
    }

    /**
     * Build index for "CommandCodeStatisticsEntry".
     */
    public SnmpIndex buildSnmpIndex(CommandCodeStatisticsEntryMBean entry)
        throws SnmpStatusException {
        SnmpOid[] oids = new SnmpOid[3];
        SnmpValue val = null;
        val = new SnmpGauge(entry.getPeerIndex());
        oids[0] = val.toOid();
        val = new SnmpGauge(entry.getApplicationID());
        oids[1] = val.toOid();
        val = new SnmpGauge(entry.getCommandCode());
        oids[2] = val.toOid();
        return new SnmpIndex(oids);
    }

    /**
     * Build index for "CommandCodeStatisticsEntry".
     */
    public SnmpOid buildOidFromIndex(SnmpIndex index)
        throws SnmpStatusException {
        SnmpOid oid = new SnmpOid();
        if (index.getNbComponents() != 3)
            throw new SnmpStatusException(SnmpStatusException.noSuchInstance);
        try {
            Vector v = index.getComponents();
            SnmpGauge.appendToOid((SnmpOid)v.elementAt(0), oid);
            SnmpGauge.appendToOid((SnmpOid)v.elementAt(1), oid);
            SnmpGauge.appendToOid((SnmpOid)v.elementAt(2), oid);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.noSuchInstance);
        }
        return oid;
    }

    /**
     * Build index for "CommandCodeStatisticsEntry".
     */
    public SnmpOid buildOidFromIndexVal(Long aPeerIndex, Long aApplicationID, Long aCommandCode)
        throws SnmpStatusException  {
        SnmpOid oid = new SnmpOid();
        try {
            SnmpGauge.appendToOid(new SnmpGauge(aPeerIndex).toOid(), oid);
            SnmpGauge.appendToOid(new SnmpGauge(aApplicationID).toOid(), oid);
            SnmpGauge.appendToOid(new SnmpGauge(aCommandCode).toOid(), oid);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.noSuchInstance);
        }
        return oid;
    }

    /**
     * Build index for "CommandCodeStatisticsEntry".
     */
    public SnmpIndex buildSnmpIndex(long[] index, int start)
        throws SnmpStatusException {
        SnmpOid[] oids = new SnmpOid[3];
        int pos = start;
        oids[0] = SnmpGauge.toOid(index, pos);
        pos = SnmpGauge.nextOid(index, pos);
        oids[1] = SnmpGauge.toOid(index, pos);
        pos = SnmpGauge.nextOid(index, pos);
        oids[2] = SnmpGauge.toOid(index, pos);
        return new SnmpIndex(oids);
    }


    /**
     * Factory method for "CommandCodeStatisticsEntry" entry MBean class.
     * 
     * You can redefine this method if you need to replace the default
     * generated MBean class with your own customized class.
     * 
     * @return An instance of the MBean class generated for the
     *         "CommandCodeStatisticsEntry" conceptual row.
     * 
     * Note that when using standard metadata,
     * the returned object must implement the "CommandCodeStatisticsEntryMBean"
     * interface.
     */

    public Object createCommandCodeStatisticsEntryMBean(SnmpMibSubRequest req,
                SnmpOid rowOid, int depth, ObjectName entryObjName,
                SnmpMibTable meta, Long  aPeerIndex, Long  aApplicationID, Long  aCommandCode)
            throws SnmpStatusException  {

        // Note that when using standard metadata,
        // the returned object must implement the "CommandCodeStatisticsEntryMBean"
        // interface.
        //
        CommandCodeStatisticsEntry entry = new CommandCodeStatisticsEntry(theMib);
        entry.PeerIndex = aPeerIndex;
        entry.ApplicationID = aApplicationID;
        entry.CommandCode = aCommandCode;
        return entry;
    }


    /**
     * Reference to the MBean server.
     */
    protected MBeanServer server;

}
