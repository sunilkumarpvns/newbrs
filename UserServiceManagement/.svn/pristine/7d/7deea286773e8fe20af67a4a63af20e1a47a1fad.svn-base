...
logger.info("response from validateOTP : "+response); 
			...
			if(CollectionUtils.isNotEmpty(authDetails)) {
				...
				if(status.equals(StatusEnum.STATUS_ACTIVE)){
					...
					if(viewProfileRQ.getRole().equals(RolesEnum.PASSENGER.name())) {
						...
						Object[] obj=passengerProfileDAO.getPassengerProfile(userId);
						if(obj!=null && obj.length>0) {
						//get ViewProfile response
						ViewProfileRS viewProfileRS = getViewProfileRS(obj);
						//generate response
						response.setStatus(ResponseEnum.SUCCESS.getValue());
						response.setMsg("Passenger Details Found");
						response.setData(viewProfileRS);
						}else {
							response.setStatus(ResponseEnum.FAILURE.getValue());
							response.setMsg("Passenger Not Found");
	...
	private ViewProfileRS getViewProfileRS(Object[] obj) {
		ViewProfileRS viewProfileRS=new ViewProfileRS();
		viewProfileRS.put("userId",obj[0]);
		viewProfileRS.put("firstName",obj[1]);
		viewProfileRS.put("lastName",obj[2]);
		viewProfileRS.put("gender",obj[4]);
		viewProfileRS.put("dob",obj[3]);
